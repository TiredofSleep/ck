...
def curvature_features(text: str) -> Dict:
    ...
    if n < 3:
        return {
            'n_letters': n,
            'mean_d2': np.zeros(NDIMS, dtype=np.float32),
            'var_d2': np.zeros(NDIMS, dtype=np.float32),
            'd2_magnitude_mean': 0.0,
            'd2_magnitude_std': 0.0,
            'operator_dist': np.zeros(10, dtype=np.float32),
            'dominant_op': -1,
    ...
    dominant_op = int(np.argmax(op_dist)) if len(ops) > 0 else -1
...
def _classify_d2(d2: np.ndarray) -> int:
    ...
    # Find dominant curvature dimension
    abs_d = np.abs(d2)
...
def coherence_score(text: str) -> float:
    ...
    # Component 1: Operator concentration (0-1)
    # If operators are spread evenly (all 0.1), concentration is low.
    # If dominated by 1-2 operators, concentration is high.
...
# ═══════════════════════════════════════════════════════════
# 4D FPGA PROJECTION
# For Zynq deployment: project 5D → 4D to fit DSP slices
# ═══════════════════════════════════════════════════════════
