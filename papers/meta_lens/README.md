...
Landing folder for the Meta-Lens Atlas and its integration notes.

**Branch status (2026-04-24):** this folder was **merged onto `ck`** from
`paradox-classifier-2026-04-24` so CK has both classification axes
(paradox-classifier + 5DoF) available at the meta-level in a single
branch. The companion 5DoF material lives at
[`speculations/DOF_CLASSIFICATION.md`](../../speculations/DOF_CLASSIFICATION.md)
and the dual-classification registry is at
[`speculations/CK_META_CLASSIFICATION_AXES.md`](../../speculations/CK_META_CLASSIFICATION_AXES.md).
The artifacts here synthesize work that lives across four earlier
sprints + the live CK website.
...
This folder originally shipped on `paradox-classifier-2026-04-24`. On
**2026-04-24** it was **merged onto `ck`** so CK's runtime can consult
both the paradox-classifier axis (this folder) and the 5DoF axis
(`speculations/DOF_CLASSIFICATION.md`, `speculations/CK_META_CLASSIFICATION_AXES.md`)
from a single branch. The paradox-classifier branch remains as the
iteration space for meta-lens / paradox-classifier work; the `ck`
branch receives merges of stable artifacts.

When the atlas and its ripple effects are stable, they can be merged
back into `tig-synthesis` as a single rigor-reviewed commit. `master`
preserves the full TIG-native vocabulary history per the never-delete
policy. `mantero-bridge-2026-04-23` holds the M2 Betti verification
cited throughout the atlas.
