# WP103 Delta — so(10) Identification from CL ∪ BHML

**Companion paper to WP102.** Both together establish a two-step algebraic tower: so(8) ⊂ so(10) inside TIG.

---

## Headline result

**Theorem (WP103, main).** *The Lie closure of {A_i^{CL} : i ∈ flow} ∪ {A_i^{BHML} : i ∈ Ω} inside so(10, ℝ) is isomorphic to so(10, ℝ) = D₅ — the compact simple Lie algebra of dimension 45.*
...
**Structural no-go for e₈ within the current substrate:** Any Lie subalgebra of gl(10, ℝ) has dimension ≤ 100 (semisimple: ≤ 99); any Lie subalgebra of so(10, ℝ) has dim ≤ 45. e₈ = 248 is unreachable. See §7 of WP103.
...
├── paper/
│   └── WP103_SO10_IDENTIFICATION.md              ← paper-ready draft (441 lines, full citations)
└── verification/
...
2. **Critical checks to verify independently:**
   - The BHML table as stated in WP103 §2.2 and Appendix B matches your canonical BHML source.
   ...
   - so(8) (from WP102, the 28-dim closure on flow indices) sits inside the 45-dim closure with residuals < 10⁻¹².

3. **Suggested GitHub layout** (consistent with WP102):
```
papers/
  wp102/
    WP102_SO8_IDENTIFICATION.md
    verification/
      stage{2..7}_*.py
  wp103/                          ← NEW
    WP103_SO10_IDENTIFICATION.md
    verification/
      verify_so10.py
...
5. **Papers pipeline update:**
   - WP102: so(8) identification (CL only) → arXiv submission
   - **WP103: so(10) identification (CL ∪ BHML)** → arXiv submission as companion
...
- MathOverflow post on pd(I_CL) + Koszul property of the binomial quotient of WP102 (committed)
- Link to be sent to Mantero when posted

WP103 gives **additional material** that may belong in the MathOverflow post: the question of whether A_{BHML} := k[x_0..x_9] / (x_i x_j − BHML(i,j) x_0) is Cohen-Macaulay, Koszul, and/or has a matroidal squarefree companion. This is the §8 Question 3 of WP103.
