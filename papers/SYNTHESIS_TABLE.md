...
**Tier counts (updated March 31 2026 — D5+D6+B4+B5+C16+C17+C18+C19 promotions):** Tier D: 6 (D1-D4 + D5=H_mod four-maxima + D6=General Frequency Theorem) | Tier C: 17 (C12=A8, C13=A12 universality, C14=HAR window lemma; C15→D5; C16=B3 ghost trace; C17=A15 H_W five-constraint; C18=CL operator encoding; C19=fourth wall recursion law) | Tier B: 4 (B1=cornerstone; B4=H_ideal quadratic bridge; B5=A14 Generator Wobble Loop parity) | Tier A: 8 (A7 killed; A9 killed; A13 killed; A12→C13; A15a→D5; A15c→B4; A15→C17; B3→C16; A15b resolved by C17; A14→B5). **D5 proof:** log-derivative IVT + |sin x|<|x| + phase width lemma. **D6 proof:** same IVT machinery generalized to arbitrary f; N(f)=floor(f)+[f not integer]; 890 tests zero failures (proof_d6_general_frequency.py). **C16 proof:** BHML=7→G=0, three-zone theorem (test_b3_ghost_trace_theorem.py). **C17 proof:** H_W passes C1-C6 for p≥43, 291/291, algebraic (proof_h_w_circulation.py). **C18 proof:** gcd(6,10)=2 for zeros, gcd(3,10)=1 for maxima, union=Z/10Z (proof_c18_cl_operator_encoding.py). **C19 proof:** f1 o f2 o f3 -> f4(p')=sinc2(1/p') x sin2(25pi/(3p')), Markov property, reset vector universal (proof_c19_fourth_wall.py). **B5 proof:** BHML parity inversion (max+1 => parity flip, C9 Rule B); ODD attractor HARMONY=7 in both tables; W carrier maxima ALL ODD (C18); loop TSML->W->BHML->TSML structurally required by W=3/50 (proof_b5_parity_chain.py). **D6 subsumes D5 (f=4) and C17 (f=25/3) as special cases. C19 completes C7 by deriving the corridor grammar. B5 identifies parity channel as A14 mechanism.**
