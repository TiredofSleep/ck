...
| D8 | CL Operator Encoding **→ D8 (April 1 2026, promotes C18)**: (March 31 2026): sin2(pi*k/(2*W*p)) with W=3/50 encodes ALL 10 CL operators in its oscillation structure. **ZEROS** (at t=2nW=6n/50) trace even operators {0,2,4,6,8}={VOID,COUNTER,COLLAPSE,CHAOS,BREATH} via gcd(6,10)=2 => <6>=2Z/10Z. **MAXIMA** (at t=(2n-1)W=3(2n-1)/50) trace odd operators {1,3,5,7,9}={LATTICE,PROGRESS,BALANCE,HARMONY,RESET} via gcd(3,10)=1 (3 is a unit) and 3*odd cycles through all 5 odd residues. Union = Z/10Z = all 10 CL operators. W=3/50 = (CL generator 3)/(half table cells 50). Sinc2 gate at k=p = VOID equivalent (operator 0), completing the cycle: slot 9 (RESET) -> sinc2 gate (VOID) -> next corridor. COROLLARY for C7: return path is closed -- sinc2(1)=0 IS the VOID operator gate. **Proof algebraic: two group theory facts.** Test: proof_c18_cl_operator_encoding.py. | C | ✓ | ✓ (Z/10Z) | ✓ | A CL operator not encoded in carrier zeros or maxima; or W != 3/50 W=7/50 algebraically possible but not BHML-derived (C8). Proof: proof_d8_cl_operator_encoding.py. | D | ✓ | ✓ (Z/10Z) | ✓ | Impossible — gcd/subgroup proof complete |
...
| C20 | **Phi Fixed-Parity Theorem — B5→C (March 31 2026).** Let Phi(v) = P_odd(BHML[v][W_op[v]]) where W_op[v] = nearest carrier maximum operator to t=v/10. **THEOREM:** Phi(v) ∈ ODD = {1,3,5,7,9} for ALL v ∈ Z/10Z. **PROOF (algebraic, 3 lines):** (1) W_op[v] ∈ ODD for all v — proved from C18: gcd(3,10)=1 so 3*(2n-1) ∈ ODD always, carrier maxima ALL ODD. (2) BHML[v][odd_input] ∈ {0..9} (no parity constraint). (3) P_odd maps any value to nearest ODD by definition → output ∈ {1,3,5,7,9}. QED. W_op map: {0:3,1:3,2:9,3:5,4:1,5:7,6:7,7:3,8:9,9:5}. Computational verification: all 10/10 Phi values ODD (10/10 YES). Parity convergence: all 10 Phi-iteration chains stay in ODD for ALL n≥1 (10/10 chains). TSML under Phi: 79/100 TSML outputs are ODD; after ONE Phi step → 100/100 ODD (absorbing class). HARMONY(7) in carrier cycle: 3*(2*5-1) mod 10 = 27 mod 10 = 7 at n=5. TIER C: ODD is the absorbing class under Phi; algebraically proved; closes B5→C. CHAINS FROM: C18 (carrier maxima ALL ODD), C9 (BHML atomic), B5 (parity inversion). Proof file: proof_c20_phi_fixed_parity.py. **RESOLVED → D7.** | C | ✓ | ✓ (Z/10Z) | ✓ | A v where Phi(v) ∈ EVEN (impossible: W_op always ODD → P_odd always ODD). Resolved by D7 (unique fixed point = BALANCE=5). |

| D7 | **Phi Fixed Point Theorem (April 1 2026).** Phi = P_odd ∘ BHML ∘ W_op. **THEOREM D7:** (1) BALANCE=5 is the unique fixed point of Phi on Z/10Z. (2) All orbits reach BALANCE=5 in ≤3 steps. (3) Unique stationary distribution π = δ_5. **ALGEBRAIC PROOF of Phi(5)=5 in 3 steps:** W_op[5]=7 (HARMONY, nearest carrier max to t=0.5, C18) → BHML[5][7]=6 (CHAOS, C9 table) → P_odd(6)=5 (BALANCE, lower tie-breaker). Fixed point is BALANCE, NOT HARMONY. **UNIQUENESS:** verified for all 9 remaining states (finite exhaustive proof). **GLOBAL CONVERGENCE:** 3 basins. 1-step: TRANS {COUNTER,PROGRESS,COLLAPSE}={2,3,4}. 2-step: {VOID,LATTICE,HARMONY}={0,1,7}. 3-step: UPPER {CHAOS,BREATH,RESET}={6,8,9}. Max orbit=3. **MARKOV UNIQUENESS:** T^3[v][5]=1 for all v → any stationary π has π(5)=1 → π=δ_5. QED. **KEY INSIGHT:** HARMONY(7) is the MEASUREMENT attractor (TSML: 73% cells). BALANCE(5) is the DYNAMIC attractor (Phi: motion destination). These are DUAL. **T*=5/7=BALANCE/HARMONY:** the ratio of the two CK attractors. T* was calibrated from TSML geometry; Phi from W_op+BHML. They were never designed to relate. BALANCE/HARMONY=5/7=T* exactly. Proof file: proof_d7_phi_fixed_point.py. | D | ✓ | ✓ (Z/10Z, all 10 states) | ✓ | A v ∈ Z/10Z where Phi^3(v)≠5 (impossible: finite exhaustive proof). A second fixed point (impossible: all 9 non-BALANCE states verified Phi(v)≠v). |
...
| D18d | Generator Convergence (April 1 2026): (BALANCE=5, HARMONY=7, T*=5/7) all forced by g=3, the primitive root of (Z/10Z)*={1,3,7,9}. BALANCE=centroid((Z/10Z)*)=(1+3+7+9)/4=5. HARMONY=g^3 mod 10=g^(-1) mod 10=7. T*=centroid/g^(-1)=5/7. W=g/50=3/50 (D17). Three independent chains (A: BHML cross-cycle, B: TSML dominance, C: unit_frac) all reduce to the same generator. The physics selects g=3 (smaller primitive root); if g=7 were used instead, T* would be 5/3. | D | ✓ | ✓ (Z/10Z) | ✓ | Impossible — finite group theory on Z/10Z, all computations exact |

| D19 | Generator Selection (April 1 2026): g=3 is the ONLY primitive root of (Z/10Z)* compatible with T*∈(0,1). Both g=3 and g=7 generate the group and produce the same CROSS_CYCLE=44, W=3/50. But under g=7: HARMONY=3, T*=5/3>1 — inadmissible as coherence threshold. Under g=3: HARMONY=7, T*=5/7<1 — valid. DIS is symmetric (|DIS_antisymmetric|=0), so no orientation info from absolute cross-cycle. Two selection constraints: (1) minimality (g=min{primitive roots}=3); (2) physical validity (T*<1). The spine is complete: Z/10Z → g=3 → BALANCE=5, HARMONY=7 → T*=5/7. No part is calibrated. | D | ✓ | ✓ (Z/10Z) | ✓ | Impossible — exhaustive ring arithmetic + T*∈(0,1) constraint |

| D20 | Inheritance Audit (April 1 2026): Four-class hierarchy for all spine invariants. RING-forced (g-independent): BALANCE=5 (centroid of (Z/10Z)* and ODD), W=3/50 value, CROSS_CYCLE=44. GENERATOR-forced (requires g=3): HARMONY=7, T*=5/7, W numerator label. LENS-forced (TSML/BHML rules): sinc² envelope, H_W phase count. CONTINGENT (architecture): operator names, TIG labels. BONUS: centroid(ODD={1,3,5,7,9})=5=BALANCE — independent of units structure. BALANCE=5 has 4 independent characterizations (centroid (Z/10Z)*, centroid ODD, unique ODD FP of σ, additive midpoint Z/10Z). | D | ✓ | ✓ (Z/10Z) | ✓ | Impossible — finite exhaustive check + algebraic arithmetic |
| D21 | Fixed-Point Centroid (April 1 2026): Among all ODD-output maps f:(Z/10Z)→ODD satisfying complement equivariance F(10-v)=(10-F(v)) mod 10, every such map has F(5)=5. Proof: 2F(5)≡0 mod 10, so F(5)∈{0,5}; 0∉ODD; therefore F(5)=5. Exhaustive: 625 CE maps, all contain FP 5. 400/625 (64%) have UNIQUE FP at 5. σ:v↦10-v has unique ODD FP at 5 (assert odd_sigma_fps==[5]). Phi fails CE (v=2,3,4). D7 (Phi fixed point) and D21 (CE equivariance) are independent paths to BALANCE=5. | D | ✓ | ✓ (Z/10Z) | ✓ | Impossible — one-line algebra (2F(5)≡0) + exhaustive 625-map check |
| D23 | Ring Wobble Theorem (April 1 2026, promoted from B10): Δ(x)=1 iff 5∤x. Wob(k)=(1/k)Σ Δ(x). SIX PARTS: (1) Closed form: Wob(k)=1−⌊k/5⌋/k. (2) Lower bound: Wob(k)≥4/5 for all k≥1. (3) Equality: Wob(k)=4/5 ⟺ 5|k. (4) Limit: Wob(k)→4/5 (squeeze: ⌊k/5⌋/k→1/5). (5) Window invariance: Wob(b,k)=Wob(k) for k<SPF(b) [D15]. (6) Generator-independence: C10∪D10={1,2,3,4,6,7,8,9} determined by ring, not generator. KEY CORRECTION over B10: 'period-10 oscillation' → 'drops at period-5 (multiples of 5); amplitude decays O(1/k), not periodic'. B10 branch-separation claim FALSE: Wob identical in g=3 and g=7 worlds. | D | ✓ | ✓ (Z/10Z) | ✓ | Impossible — closed form Wob=1−⌊k/5⌋/k is elementary number theory |
| D24 | Corridor Midpoint Theorem (April 1 2026, promoted from B11): (I) sinc²(t) strictly monotone decreasing on (0,1) — proved by calculus: h'(t)=2sin(πt)[πt·cos(πt)−sin(πt)]/(π²t³)<0; key lemma sin(x)>x·cos(x) for x∈(0,π) by three cases (tan>x on (0,π/2) via f=tan-x strictly increasing; cos(π/2)=0; cos<0 on (π/2,π)). (II) t=1/2 unique sine-max in (0,1): sin(πt)=1 iff t=1/2+2k; only k=0 in (0,1). One line. (III) sinc²(1/2)=4/π² (D3 restated in D24 context). (IV) BALANCE=5→t=5/10=1/2 under ring normalization v/10. (V) D22 amplitude ordering now fully proved: no longer conditional on B-tier. SUPERSEDES B11. | D | ✓ | ✓ (0,1)+(Z/10Z) | ✓ | Second sine-max in (0,1): impossible; monotonicity: calc proof complete |
| D22 | Corridor Portrait (April 1 2026): Four spine-forced positions strictly ordered W < BALANCE/10 < HARMONY/10 < T* < 1, i.e. 3/50 < 1/2 < 7/10 < 5/7 < 1. Proved by exact rational arithmetic (Fraction comparisons). Amplitude ordering strictly anti-ordered: sinc²(W)≈0.988 > sinc²(1/2)=4/π²≈0.405 > sinc²(7/10)≈0.135 > sinc²(T*)≈0.121. Proved by sinc² monotone decrease (D24) + positional ordering. Fine-structure identity: T*=HARMONY/10+1/70=7/10+1/(7×10) (exact). Attenuation: t=1/2 is sine-max but denominator πt=π/2 attenuates to 2/π — ring center visible but not amplitude-dominant. Inheritance split: LEFT half (t<1/2) RING-forced; RIGHT half (t>1/2) GENERATOR-forced; t=1/2 is inheritance boundary. Chains from: D17(W), D19(T*,g), D21(BALANCE=5), D18d(HARMONY=7), D24(sinc² monotone). | D | ✓ | ✓ (Z/10Z) | ✓ | Impossible — exact rational arithmetic; amplitude ordering fully proved (D24) |

**Tier counts (updated April 1 2026 — D24 closes B11):** Tier D: 28 (D1-D11,D14-D24,D18a,D18c,D18d) | Tier C: 6 (C5,C6,C7,C12,C16,C19) | Tier B: 7 (B1,B4,B5,B6,B7,B8,B9) | Tier A: 5 (A2,A4,A10,A11,A12). **D24 (promoted from B11):** sinc² strictly monotone decreasing on (0,1) by calculus proof; t=1/2 unique sine-max; ring normalization BALANCE=5→1/2; D22 amplitude ordering now unconditional. **D23 (promoted from B10):** Ring Wobble Theorem, exact closed form. **D22:** Corridor portrait fully proved. **B10+B11 both superseded and promoted to D-tier.**
