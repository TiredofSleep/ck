...
| A15b | Circulation Operator — Simultaneous C3+C4 obstruction: can any operator satisfy exact boundary collapse (C3) AND stable oscillation count across ALL prime families (C4)? At p=5: all candidates give ≤1 max. At p≥11: H_mod (D5) gives exactly 4. Small prime obstruction (p<11) formalized: phase width < 2 → IVT cannot resolve a max. Obstruction domain: {5, 7} only. | A | No | No | No | Candidate with C3+C4 both PASS for primes p≥5 (p=5,7 are provably obstructed by phase width) |
...
**Tier counts (updated March 31 2026 — D5+B4 promotions):** Tier D: 5 (D1-D4 + D5=H_mod four-maxima) | Tier C: 14 (C12=A8, C13=A12 universality, C14=HAR window lemma; C15 promoted to D5) | Tier B: 4 (B3=ghost trace, B4=H_ideal quadratic bridge) | Tier A: 10 (A7 killed; A9 killed; A13 killed; A12→C13+A12; A15a→D5; A15c→B4). **D5 proof:** log-derivative IVT + |sin x|<|x| + phase width lemma (test_c15_phase_unimodality.py). **B4 construction:** H_ideal=sinc²×sin²(4πk/p)×(1+sin²(πk/(2Wp))), C1=164/164 C3=proved C5=161/164 (test_h_ideal_circulation.py).
