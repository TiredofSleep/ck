...
| B4 | Circulation Operator — Hybrid construction: H_mod=sinc²(k/p)×sin²(4πk/p). BOUNDARY CORRECTED (March 31 2026): original test used vals[1:p] slice that excluded k=1 from max check, creating false p=13 boundary. Correct counting: k is a local max if H(k)>H(k-1) AND H(k)>H(k+1) for k∈{1,...,p-1}. With correct counting: C1 (4+ maxima) holds for p≥11 (verified 23 primes to p=97). p=5,7 fail (2-element phase 2 is too narrow for local max). C2env (upper envelope r>0.9 with sinc²): verified across all primes. C3: sinc²(1)=0 → H_mod(p,p)=0 provably. C4 stable: exactly 4 maxima for p≥11 (verified). C5: first max at k≈p/8 (t≈1/8, not W — C5 partial). Why p=7 fails: phase 2 has width 2 (k=4,5), monotone declining from phase 1 peak through phase 2. Why p=11 passes: phase 2 has width 3 (k=6,7,8), one interior max formed at k=7. Tier B: 4 constraints met computationally for p≥11; C3 proved. | B | ✓ | ✓ (p≥11) | Partial | Algebraic proof that phase 2 always contains a max for p≥11; algebraic C5 (first max at t=W exactly); C2 raw proof or justification for envelope criterion |
...
**Tier counts (updated March 31 2026):** Tier D: 4 | Tier C: 14 (C12=A8, C13=A12 universality, C14=HAR window lemma) | Tier B: 4 (B3=A16 three-zone, B4=A15a hybrid construction p≥11) | Tier A: 11 (A7 killed; A9 killed; A13 killed; A12→C13+A12; A15a→B4; A15→A15b/c remaining)
