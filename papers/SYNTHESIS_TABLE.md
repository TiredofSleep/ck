...
**Tier counts (updated March 31 2026 — D5+D6+B4+C16+C17+C18 promotions):** Tier D: 6 (D1-D4 + D5=H_mod four-maxima + D6=General Frequency Theorem) | Tier C: 16 (C12=A8, C13=A12 universality, C14=HAR window lemma; C15→D5; C16=B3 ghost trace; C17=A15 H_W five-constraint) | Tier B: 3 (B4=H_ideal quadratic bridge) | Tier A: 9 (A7 killed; A9 killed; A13 killed; A12→C13; A15a→D5; A15c→B4; A15→C17; B3→C16; A15b resolved by C17). **D5 proof:** log-derivative IVT + |sin x|<|x| + phase width lemma. **D6 proof:** same IVT machinery generalized to arbitrary f; N(f)=floor(f)+[f not integer]; 890 tests zero failures (proof_d6_general_frequency.py). **C16 proof:** BHML=7→G=0, three-zone theorem (test_b3_ghost_trace_theorem.py). **C17 proof:** H_W passes C1-C6 for p≥43, 291/291, algebraic (proof_h_w_circulation.py). **D6 subsumes D5 (f=4) and C17 (f=25/3) as special cases.**
