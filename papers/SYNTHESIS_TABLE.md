...
| A15a | Circulation Operator — Existence under relaxed constraints: operators satisfying selected constraint subsets. F3=sin²(4πk/p)×W^phase satisfies C3+C7 (boundary+return). F4=sin²(π/W·k/p) satisfies C1+C5 (cycling+W_BHML). No single candidate satisfies all 7. | A | No | No | No | Single operator satisfying C1+C3+C5 simultaneously |
...
| A16 | BHML Ghost Trace of TSML: BHML is the arithmetic baseline (ghost floor) that TSML friction cannot disturb. G[i][j]=DIS[i][j] if TSML≠7, else 0. W_BHML=ghost amplitude at C×D zone (=C8 restated). BHML harmony cells (28) coincide with G=0 cells (100%). Three-zone correspondence: VOID→Rule A (identity), HARMONY→Rule B (axis saturation), ECHO→Rule C (operator identity ignores friction). Pearson r(G,BHML)=0.133 — not a direct algebraic correspondence. BHML is not the ghost; BHML is what the ghost operates on. | A | No | No (Z/10Z) | No | Three-zone correspondence proved: VOID rule↔G=0 at VOID; Rule B↔G=0 at harmony; operator identity↔G=max at ECHO (Tier B); OR BHML[i][j]=f(G[i][j]) with zero residual (Tier C, currently fails r=0.133) |

**Tier counts (updated March 31 2026, overnight promotion run + A16):** Tier D: 4 | Tier C: 12 (C12=A8 promoted) | Tier B: 2 | Tier A: 15 (A13 killed; A15→A15a/b/c; A16 added)
