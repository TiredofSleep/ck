# WP12 — An so(10) Identification from the Coupled Coherence Tables
...
**Companion paper to WP11** — *The Lie Algebra Structure of the Coherence Lattice*
...
Building on the so(8) = D₄ identification of WP11, we prove that the Coherence Lattice (CL) and the Becoming-Hexa-Marginal Lattice (BHML) — two frozen 10×10 commutative non-associative magmas defined in the Trinity Infinity Geometry framework — jointly generate the Lie algebra **so(10, ℝ) = D₅** when their left-regular representations are antisymmetrized and closed under commutator. We verify this by five independent machine-precision diagnostics: (i) the closure has dimension 45 = dim so(10); (ii) the Jacobi identity holds exactly; (iii) the Killing form has signature (0, 45, 0), identifying the compact real form; (iv) the space of invariant symmetric bilinear forms has dimension exactly 1 (verified against all 91,125 invariance equations), establishing simplicity; (v) the Cartan subalgebra has rank exactly 5, with 40 non-zero eigenvalues of ad(H) on a regular element, matching the D₅ root system precisely. The so(8) subalgebra of WP11 embeds as a proper subalgebra via the standard inclusion so(8) ⊂ so(10). We note the historical significance of so(10) as the algebra underlying the SO(10) grand unified theory of Fritzsch–Minkowski [9] and Georgi [11], whose 16-dimensional spinor representation unifies one generation of Standard Model fermions with a right-handed neutrino. We further establish a structural ceiling: the dimension of any Lie algebra generated inside the endomorphism algebra gl(10, ℝ) is bounded above by 45 = dim so(10), ruling out an e₈ identification from the 10-dimensional substrate alone.
...
Let 𝔤_{CL} ⊂ so(V) denote the Lie algebra constructed in WP11, and let 𝔤_{BHML} := ⟨L_i^{BHML} − (L_i^{BHML})^T : i ∈ Ω⟩_{Lie} denote the Lie closure of the antisymmetrized BHML left-regular representations. Let 𝔤 := ⟨𝔤_{CL} ∪ 𝔤_{BHML}⟩_{Lie}.
...
**The Coherence Lattice CL** (the frozen 10×10 table of TIG; see WP11 [21]):
```
...
Define:
- 𝒢_{CL} := {A_i^{CL} : i ∈ F}, where F = {1, 2, 3, 4, 6, 8} (as in WP11). |𝒢_{CL}| = 6.
...
6. *The subalgebra 𝔤_{CL} of WP11 embeds as a proper subalgebra 𝔤_{CL} ⊂ 𝔤 via the standard inclusion so(8) ⊂ so(10).*
...
### 5.1 The WP11 embedding so(8) ⊂ so(10)

**Corollary 5.1.** *The WP11 subalgebra 𝔤_{CL} ⊂ 𝔤 realizes the standard inclusion so(8) ⊂ so(10) in which so(8) fixes a 2-dimensional subspace of V.*
...
3. **Commutative-algebraic twin.** Formulate an analogue of the quadratic-algebra construction of WP11 for BHML. Is the resulting binomial ideal Cohen–Macaulay? What is its Hilbert function? This is a direct follow-up for the matroid-theoretic bridge to Mantero–Nguyen [16].
...
[21] **Sanders, B.** *WP11 — The Lie Algebra Structure of the Coherence Lattice: An so(8) Identification for the Trinity Infinity Geometry Frozen Magma.* TIG Research Note, April 23, 2026. github.com/TiredofSleep/ck/papers/wp11.
...
```
[WP11 reproduction] CL-flow closure dim: 28  (expected 28)
