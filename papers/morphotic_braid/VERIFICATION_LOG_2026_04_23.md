...
> **Scope note on Claim 4:** `det = 70 = 2·5·7` holds for the 8×8 core `BHML_8` (WP15 §0). The full 10×10 `BHML_10` has `det = −7002`, primes `{2, 3, 389}`. Both values are independently verified (SymPy exact integer). Claim 4 below is GREEN for its intended scope (the WP15 core) once the scope is named. See `FORMULAS_AND_TABLES.md` §6.7.
