...
> **Lens-dependence note (REVISED 2026-05-06 night):** The original preprint (2026-04-26) claimed a 7-element chain forbidding sizes $\{2, 3, 7\}$. A rebuild of the brute-force enumeration on 2026-05-05 during preparation of the four-core consolidated paper (Sanders + Gish, in prep., *Algebraic Combinatorics*) found 8-element chain with forbidden $\{2, 3\}$, and the WP115 source was updated above. Subsequent re-verification on 2026-05-06 night (per `Atlas/LENS_TAXONOMY_2026-05-06/TIER_CONFLATION_AUDIT.md` M4) shows that **the 7- vs 8-element chain count is lens-dependent**: the 7-element chain holds on the literal bit-pattern $T_{\mathrm{RAW}}$ (the non-commutative variant of CL_TSML), where the asymmetric cells $T[9,4]=3$ and $T[3,9]=3 \neq 7 = T[9,3]$ break closure of the size-7 shell $\{0,4,5,6,7,8,9\}$. The 8-element chain holds on the upper-triangle symmetrized $T_{\mathrm{SYM}}$ (where commutativity is restored). Both are valid; they describe different lens choices on the same canonical bit-pattern encoding. The original WP115 result was correct on $T_{\mathrm{RAW}}$; the four-core consolidated paper uses $T_{\mathrm{SYM}}$ and reports the 8-element chain. Theorem 2 (universal 4-core attractor at $\alpha=1/2$) and Theorem 3 (α-endpoint structure) are lens-invariant because the 4-core $\{V, H, Br, R\}$ is itself lens-invariant. Going forward, papers should explicitly scope which TSML lens is used; the present chain claim is given on $T_{\mathrm{SYM}}$ for consistency with the four-core consolidated paper's framing.
