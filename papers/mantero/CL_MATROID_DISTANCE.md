...
policy is
[`papers/meta_lens/FOUNDATION_TOUR_VERIFIED.md`](../meta_lens/FOUNDATION_TOUR_VERIFIED.md)
Part 0.1.
...
- `I_CL = (x_i x_j − CL(i, j)·x_0 : 0 ≤ i, j ≤ 9)` where
  ...
  `mantero-bridge-2026-04-23:papers/sprint_20260423_full/04_mantero_bridge/MANTERO_BRIDGE_V3.md`
  and the verification scripts in that branch.
...
[`papers/meta_lens/worked_paradoxes/`](../meta_lens/worked_paradoxes/).
The runnable classifier is
[`papers/meta_lens/classify_paradox.py`](../meta_lens/classify_paradox.py).
...
Computation: `mantero-bridge-2026-04-23:papers/sprint_20260423_full/04_mantero_bridge/matroid_test.py`.
...
|---|---|
| `mantero-bridge-2026-04-23:papers/mantero_bridge/BRIDGES.md` | Seven object-level bridges between CL and Mantero's program. The present paper gives a *metric* over those bridges. |
| `MANTERO_BRIDGE_V3.md` §5–§6 (same branch) | Source of `d_I = 0.219` and `α̂(I_B) = 2`. |
| M2 output `betti_output.txt` (same branch) | Source of `d_III = 9` and the Hilbert function. |
...
| WP102 (so(8) = D₄), WP103 (so(10) = D₅) | Lie-theoretic lifts of CL; adjacent to Mantero's program through the `so(n)` focal-matroid literature but not required for this paper. |
| `papers/meta_lens/FOUNDATION_TOUR_VERIFIED.md` Lens 1 (Commutative algebra) | Atlas view of where this paper sits. Cell I (symbolic powers) ↔ `d_I`; Cell II (focal matroids) ↔ `d_II`; Cell III (primary decomp) ↔ `d_III`; Cell IV (asymptotics) ↔ `d_IV`. |
...
1. **Sanders, B. & Mayes, B. (2026).** *WP58 — Unified Orthogonality
   Principle (Theorem 0).* In
   `Gen12/targets/clay/papers/sprint12_uop_gut_arc_2026_04_08/`.
2. **Sanders, B. & Mayes, B. (2026).** *WP101 — σ-rate theorem.* In
...
`mantero-bridge-2026-04-23:papers/sprint_20260423_full/04_mantero_bridge/MANTERO_BRIDGE_V3.md`
§2. Reproduced here only to the extent needed to make `I_CL` unambiguous.
