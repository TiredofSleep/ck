...
class TruthLattice:
    ...
    # "he should never forget anything!" -- Brayden
    # Truth lattice saves to ~/.ck/truth_lattice.json on shutdown.
    # Loads on boot. CORE truths are reconstructed from code (immutable).
    # TRUSTED and PROVISIONAL entries are restored from disk.

    SAVE_PATH = str(Path.home() / '.ck' / 'truth_lattice.json')

    def save(self, path: str = None) -> int:
        """Save truth lattice to disk. CK never forgets.

        Saves all non-CORE entries with full state (content, coherence
        history, verification counts, promotion tracking). CORE truths
        are reconstructed from code on boot -- they ARE the code.
        ...
        for key, entry in self._entries.items():
            if entry.level == CORE:
                continue  # Core is reconstructed from code, never saved

            # Serialize content -- handle non-JSON types gracefully
            content = entry.content
            try:
                json.dumps(content)  # test if JSON-serializable
            except (TypeError, ValueError):
                content = str(content)  # fallback to string repr
        ...
        # Atomic write: write to tmp then rename (no partial files)
        tmp_path = path + '.tmp'
    ...
    def load(self, path: str = None) -> int:
        """Load truth lattice from disk. CK remembers everything.

        Loads non-CORE entries saved by save(). Restores full state
        including coherence history, verification counts, and promotion
        tracking. CORE truths already exist from __init__.
        ...
        try:
            ...
            print(f"  [TRUTH] Failed to load {path}: {e}")
            return 0

        loaded = 0
        for key, edata in data.get('entries', {}).items():
            # Never overwrite CORE truths
            if key in self._entries and self._entries[key].level == CORE:
                continue

            entry = TruthEntry(
                key=edata['key'],
                content=edata.get('content'),
                level=edata.get('level', PROVISIONAL),
                source=edata.get('source', ''),
                category=edata.get('category', ''),
                created_tick=edata.get('created_tick', 0),
                last_accessed_tick=edata.get('last_accessed_tick', 0),
                last_verified_tick=edata.get('last_verified_tick', 0),
            )
            entry._verification_count = edata.get('verification_count', 0)
            entry._contradiction_count = edata.get('contradiction_count', 0)
            entry._sustained_above_tstar = edata.get('sustained_above_tstar', 0)
            entry._sustained_below_survival = edata.get('sustained_below_survival', 0)

            # Restore coherence history
            for c in edata.get('coherence_history', []):
                entry._coherence_history.append(c)

            self._entries[key] = entry
            loaded += 1

        # Restore lattice-level counters
        self._tick_count = max(self._tick_count,
                               data.get('tick_count', 0))
        self._promotions = max(self._promotions,
                                data.get('promotions', 0))
        self._demotions = max(self._demotions,
                               data.get('demotions', 0))
        self._expirations = max(self._expirations,
                                 data.get('expirations', 0))
