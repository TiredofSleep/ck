...
A representative configuration $(\Lambda^{4}/\rho_{c,0}, \Xi_i, \Xi'_i) = (0.231, 0.925, +0.470)$ at $z_i \approx 20$ (where the prime denotes $d/dN$, the e-folds derivative) yields $w_0^{\Xi} = -0.791$, $w_a^{\Xi} = -0.492$, $\chi^{2}_{\mathrm{Gauss}} = 1.24$ (1.1σ) against the DESI 2024 DR1 published $(w_0, w_a)$ marginal Gaussian. The Type-F frozen turnaround occurs at $z_\star \approx 1.3$ — a redshift where Stage-IV $w_{\mathrm{DE}}(z)$ reconstructions (Crittenden-Pogosian-Zhao PCA, Holsclaw GP, Sahni Om-diagnostic) have substantially more data than at higher z, making the F$_5$ local-minimum-in-$w(z)$ signature *more* directly testable than would have been the case at $z_\star \approx 2$. This places the paper's central falsifiability claim in the densest part of the available data. The paper documents this fit honestly as a Gaussian-on-summary proximity measure, NOT a goodness-of-fit to the underlying joint BAO + CMB + SN likelihood (deferred to a companion numerical paper).
