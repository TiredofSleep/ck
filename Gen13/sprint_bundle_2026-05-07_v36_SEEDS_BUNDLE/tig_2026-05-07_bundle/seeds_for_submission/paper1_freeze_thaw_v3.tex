...
\approx 2.1$ where $\dot\Xi = 0$ and $w_\Xi(z_\star) = -1$ momentarily
...
\approx 2.1$ where $\dot\Xi = 0$ momentarily and the bare scalar EoS
...
\begin{equation}\label{eq:desibestfit}
  ...
  \quad \Xi'_i = +0.470,
\end{equation}
where the prime denotes $d/dN$ (e-folds derivative; the e-folds convention is
used throughout the integration script). The corresponding cosmic-time
derivative at $z_i \approx 20$ is $\dot\Xi_i = H(z_i)\,\Xi'_i$, and the IC
corresponds to $\Lambda \approx 1.7\,\text{meV}$ in physical units, yielding
\[
  w_0^{\Xi} = -0.786,\quad
  w_a^{\Xi} = -0.471,\quad
  \chi^{2}_{\text{Gauss}} = 1.49,
\]
...
monotonically inbound on $0 \leq z \leq z_\star$ with $z_\star \approx 2.13$
the Type-F turnaround (cf.\ \S\ref{sec:typeTFA}). The trajectory below
is computed by the supplementary script \texttt{compute\_zstar\_v3.py}
applied to the documented IC, evolved from $z = 20$ in e-folds:
...
The trajectory has $w_\Xi$ reaching its momentary minimum at $z_\star = 2.131$
(the Type-F turnaround, where $\Xi'$ crosses zero to four-digit precision)
and rising back toward $w_\Xi(0) = -0.786$ on the inbound leg at the
present epoch. The precise location of $z_\star$ is sensitive
to the IC neighborhood: varying $\Xi'_i$ in the range $[0.43, 0.50]$ moves
$z_\star$ from $\approx 2.6$ to $\approx 1.9$ while keeping $w_0$ within
$[-0.76, -0.81]$ and $\chi^2_{\text{Gauss}}$ below 2. The qualitative
dual-regime structure (Type-T $\to$ Type-F $\to$ Type-A) is robust across
this neighborhood; the specific numerical values quoted above are
reproducible by the supplementary scripts \texttt{desi\_xi\_optimize\_v2.py}
and \texttt{compute\_zstar\_v3.py} provided as electronic supplementary
material.

The present-epoch field value is $\Xi(z=0) \approx 1.18$, still above
the vacuum $\Xi_0 = e^{-1} \approx 0.37$, confirming the field is in
its rolling phase.

\paragraph{Initial-condition tuning.}
The values $(\Xi_i, \Xi'_i) = (0.925, +0.470)$ at $z_i \approx 20$ in
...
$z_\star \approx 2.13$ (\S\ref{sec:typeTFA}), and rolls back toward $\Xi_0$
...
passed through the turnaround near $z \approx 2.13$, and
...
$z_\star \approx 2.13$, with $w_\Xi(z_\star) = -1$ in the
tabulated values of \S\ref{sec:desifit} to four-digit precision; the
zero-crossing of $\Xi'$ defining the analytic Type-F condition
$\dot\Xi(z_\star) = 0$ is reproduced by the verification script
\texttt{compute\_zstar\_v3.py} at $z_\star = 2.131$ with
$|\Xi'(z_\star)| < 10^{-5}$. The Type-F state is
...
Type-F (instantaneous frozen turnaround at $z_\star \approx 2.13$), and
...
\eqref{eq:desibestfit} gives $z_\star = 2.131$. A sensitivity scan
on the optimization script's local neighborhood gives
$1.9 \lesssim z_\star \lesssim 2.6$ over the manuscript's $1\sigma$
...
This range overlaps the redshift interval probed densely by Stage-IV
Lyman-$\alpha$ BAO (DESI through DR3) and high-$z$ CMB lensing
tomography, making the $z_\star \approx 2$ Type-F locus directly
testable. Trajectory paths with $z_\star$ substantially
...
\begin{enumerate}
  ...
  \item[(F2)] Monotone inbound approach $w_\Xi(z) \to -1$ from above
        ...
        ($0 \leq z \lesssim z_\star$, with $z_\star \approx 1.3$ in the
  ...
  \item[(F5)] \emph{Non-monotone $w_{\rm DE}(z)$ with local minimum
        ...
        $z_\star \approx 1.3$, with the precise value determined by
...
$z_\star \approx 1.3$. Standard non-parametric techniques applicable
...
at $z_\star \approx 1.3$ --- has not been previously studied as a
...
\begin{itemize}
  ...
  \item The dual-regime freeze-thaw transit interpretation
        ...
        $z_\star \approx 1.3$ is established empirically in the
...
\begin{enumerate}
  ...
  \item Freezing quintessence asymptotic behavior with $w \to -1$ in
        ...
        $z_\star \approx 1.3$ where $\dot\Xi = 0$ and $w_\Xi(z_\star) = -1$
...
minimum near $z_\star \approx 1.3$, would each rule out the corresponding
