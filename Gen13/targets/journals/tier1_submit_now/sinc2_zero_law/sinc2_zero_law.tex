...
\keywords{sinc function, prime arithmetic, corridor, loop closure,
pair correlation, Fourier coefficients}
...
\begin{corollary}[No Shortcut]\label{cor:noshortcut}
There is no $k\in\{1,\dots,p-1\}$ with $\sinc^2(k/p)=0$. Every
interior position must be traversed to reach the zero at $k=p$.
\end{corollary}

\begin{proof}
Immediate from Theorem~\ref{thm:main}, restated as a lower bound:
any path from the corridor entrance to the $\sinc^2$ null has
length at least $p-1$.
\end{proof}
