...
%%%
%%% TODO MARKERS:
%%%   [BRAYDEN-DERIVE]    -- structural derivation needs substrate notes
%%%   [BB-BRIDGE]         -- discrete-to-continuum field theory
%%%   [INDEPENDENT-PRED]  -- testable prediction not used in fit
%%% ============================================================
...
Let $|\Aut(V)| = 40$ and $|V| = 4$ denote the automorphism-group
order and dimension of the $4$-core's $\F_5$-lift, respectively,
and let $|\sigma| = 6$ denote the $\sigma$-cycle length on
$\Z/10 \setminus \{0, 3, 8, 9\}$. Then
\[
  \Omega_{DM}
...
Theorem~\ref{thm:lambda-scale} to be explicit. Three candidate
derivations of the $1/3$ factor are:
...
\subsection{An independent prediction}\label{sec:prediction}

[INDEPENDENT-PRED: needs identification of a testable prediction
that is not used to fit the model.]

The dark-sector trinity (Theorem~\ref{thm:trinity}) and the
$\Lambda$-scale relation (Theorem~\ref{thm:lambda-scale}) are
constructed to match Planck~2018 observations. To distinguish
this framework from numerology, we require at least one
\emph{independent} prediction --- a testable observable not used
in the construction. Candidates include:
\begin{itemize}
\item A specific value of the Hubble constant $H_{0}$ derivable
from the substrate (would address the $H_{0}$ tension).
\item A specific value of the matter-clustering amplitude
$\sigma_{8}$.
\item A specific scalar spectral index $n_{s}$ from the
substrate's effective inflationary potential.
\item A specific value of the baryon-photon ratio
$\eta = n_b/n_\gamma$ from a substrate counting different from
the $\Omega_b$ derivation.
\end{itemize}
The companion sprint papers~\cite{Sprint18FrontierPredictions} (in
preparation) develop these candidates; we flag them as future
work and do not claim them in the present submission.
...
Theorem~\ref{thm:uniqueness}) is verified by the script
\texttt{sprint18\_uniqueness\_search.py} at
DOI~\texttt{10.5281/zenodo.18852047}. The script performs:
\begin{itemize}\setlength\itemsep{0pt}
\item Direct evaluation of $\Omega_b$, $\Omega_{DM}$,
$\Omega_\Lambda$ at $(H, N) = (7, 10)$;
\item Comparison to Planck~2018 published values with explicit
$\sigma$-residuals;
\item The three Hubble-independent ratio tests;
\item The uniqueness search across $784$ small-integer $(H, N)$
pairs.
\end{itemize}
The script runs to completion in under one second and prints all
