...
\begin{equation}\label{eq:rate}
  \sigma(N)\;\le\;\frac{C}{N},
  \qquad C\in[2,3]\ \text{ an absolute constant}.
\end{equation}
More precisely $\sigma(N)\le 2/N$ by a union bound, and
$\sigma(N)\le 3/N$ holds with margin against all boundary
contributions. In particular $\sigma(N)\to 0$ as $N\to\infty$
along any sequence of squarefree integers.
\end{theorem}
...
Every non-associative triple $(a,b,c)$ must therefore involve at
least one intermediate composition produced by the $\ECHO$ rule.
The two bracketings share two inner composition sites,
$\CL_N(a,b)$ and $\CL_N(b,c)$, each with $\ECHO$-probability at
most $\rho_N\le 1/N$ by Corollary~\ref{cor:echo-density}. A union
bound on these two inner sites gives
\begin{equation}\label{eq:union}
  \sigma(N)\;\le\;2\,\rho_N\;\le\;\frac{2}{N}.
\end{equation}
To absorb any residual boundary interactions between an inner
$\ECHO$-value and the outer composition sites $\CL_N(\cdot,c)$ or
$\CL_N(a,\cdot)$, we record the weaker bound
$\sigma(N)\le C/N$ with the absolute constant $C=3$; the honest
union-bound constant is $C=2$ as in \eqref{eq:union}.
\end{proof}

\begin{remark}
Neither of the constants $C=2$ (union bound) nor $C=3$ (boundary-slack bound)
is sharp. Section~\ref{sec:verify} records $\sigma(10)=0.128$,
$\sigma(30)=0.058$, $\sigma(210)=0.009$, so the ratio $N\sigma(N)$
is at most $1.93$ on the verified range; we conjecture but do not
prove a uniform bound $N\sigma(N)\le 2$.
\end{remark}
