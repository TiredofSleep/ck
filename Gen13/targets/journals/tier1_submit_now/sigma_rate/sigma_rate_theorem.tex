...
$\varphi(N)$. This $\sigma\to 0$ regime is opposite in extremum to
the $\sigma\to 1$ program of maximally nonassociative quasigroups
\cite{DrapalWanless2021,DrapalLisonek2020}, complementing rather than
competing with that family. Selected numerical examples
are verified by the script \texttt{verify\_sigma\_rate.py}, including
$N\in\{10,30,210\}$.
\end{abstract}
...
appears in the companion paper~\cite{SandersGish2026JCAP}; the
present paper does not address that limit and the combinatorial result
above is independent of it. A second companion
paper~\cite{SandersGish2026FourCore} studies the joint closure of the
specific pair $(\CL_{10}, B_{10})$ at $N=10$, including a complete
classification of subsets of $\mathbb{Z}/10\mathbb{Z}$ jointly closed
under both operations. The chain count depends on the lens choice for $\CL_{10}$: on the upper-triangle symmetrized variant the chain has 8 elements (sizes $\{1,4,5,6,7,8,9,10\}$); on the literal bit-pattern (non-commutative) variant the chain has 7 elements (size 7 forbidden). The $\sigma$-rate analysis of the present paper is independent of this lens choice — Theorem~\ref{thm:rate} concerns the family $\CL_N$ on $\mathbb{Z}/N\mathbb{Z}$ for arbitrary squarefree $N\ge 3$, and the rate bound depends only on the cell-disagreement count of the rule absorbing structure, not on any lens-symmetrization of a specific instance. The pair
$(\CL_N, B_N)$ at general $N$ is part of the same operator-substrate
family as $\CL_N$ alone but is not the subject of the present
$\sigma$-rate analysis.
...
\begin{remark}
Numerical verification via \texttt{verify\_sigma\_rate.py} confirms
$\#\{(a,b):\DIS(a,b)=0\}=\varphi(N)$ for all squarefree $N$ in
$\{2,3,5,6,7,10,11,13,15,21,30,35,42,66,77,91,105,154,210,\dots\}$,
giving in particular counts $4$, $8$, $48$ at $N\in\{10,30,210\}$.
...
\begin{remark}[The case $N=2$]\label{rem:Nis2}
At $N=2$ the harmony absorber $h=N-1=1$ coincides with the
multiplicative identity, and the absorbing structure of $\CL_N$
degenerates: every non-zero pair has the $\HARM$ rule applied. The
case-analysis arguments of \S\ref{sec:rate} require the
non-emptiness of $\{1,\dots,N-2\}$ and so are stated for $N\ge 3$.
The Echo-count Lemma~\ref{lem:echo} holds for all squarefree $N\ge 2$.
\end{remark}
...
\item[(1f)] $b,c\notin\{0,h\}$ and $\CL_N(b,c)\neq h$: write
  $m := \CL_N(b,c)$. By hypothesis $m \neq h$. The left bracketing
  is $\CL_N(\CL_N(0,b),c) = \CL_N(0,c) = 0$ by two applications
  of the $\VOID$ rule (since $b\neq h$ gives $\CL_N(0,b)=0$, and
  $c\neq h$ gives $\CL_N(0,c)=0$). The right bracketing is
  $\CL_N(0,m)$; since $m \neq h$ the $\HARM$ rule does not fire,
  so the $\VOID$ rule (with $a=0$) gives output $0$. Both
  bracketings return $0$; associative. The argument uses neither
  the squarefree hypothesis nor any restriction on the value of
  $m \in \ZN\setminus\{h\}$.
\end{itemize}
...
$(a,b,c)\mapsto(c,b,a)$ is an involution on $(\ZN)^{3}$ that swaps
the two bracketings $\CL_N(\CL_N(a,b),c)$ and $\CL_N(a,\CL_N(b,c))$.
Each rule of Definition~\ref{def:cl} is symmetric in its arguments
(both $\HARM$ and $\VOID$ trigger on either coordinate, $\ECHO$ uses
the symmetric condition $a+b\equiv ab$, and the default $\HARM$ is
constant), so the involution preserves the non-associative status of
each triple. Hence Case 2 contributes the same two-sided bound
\eqref{eq:case1-bounds} as Case 1.
...
unless $x-1$ is a unit, and in that case has at most one solution
$c\in\ZN$. The non-associative triples are then a subset of the
$\ECHO$ pairs $(a,b)$ (one per pair, when one exists), so the
count is bounded by $\#\{\ECHO\,\text{pairs}\,(a,b)\} \le \varphi(N)$
by Lemma~\ref{lem:echo}. Note that this upper bound counts every
triple $(a,b,c)$ for which the outer composition site disagrees,
including triples for which $(b,c)\in\ECHO$ may also hold; this is
an upper bound, since restricting to the (3.B) sub-subcase
hypothesis $(b,c)\notin\ECHO$ would only reduce the count.
\end{itemize}
...
\medskip
Combining the three top-level cases gives both upper and lower
bounds on $\sigma(N)\,N^{3}$. The upper bound follows from
$\#\,(\text{Case 1+2}) \le 2(N-2)^{2}$ together with
$\varepsilon(N)\le 2\varphi(N)$. The lower bound follows from
$\#\,(\text{Case 1+2}) \ge 2(N-2)^{2} - 2\varphi(N)$
(which uses $\#\,(\text{Case 1}) \ge (N-2)^{2} - E(N)$ and
$E(N)\le \varphi(N)$, summed with the analogous Case 2 inequality)
together with the trivial $\varepsilon(N)\ge 0$. Hence
...
rather than $\varphi(p)$. Direct computation for $N=2^{k}$, $k=1,\ldots,6$:
\[
\begin{array}{c|cccccc}
N=2^{k}      & 2     & 4     & 8     & 16    & 32    & 64 \\
\hline
N\,\sigma(N) & 0.000 & 0.500 & 1.250 & 1.813 & 1.922 & 2.082
\end{array}
\]
The squarefree bound $N\sigma(N) < 2$ holds at $N \in \{2, 4, 8, 16, 32\}$
but is exceeded at $N = 64$, where $N\sigma(N) \approx 2.082$. (The same
behavior continues at $N = 128$.) Empirically $\sigma(2^{k})$ decays at
rate $\sim 2^{-0.64\,k}$, slower than the squarefree $2/N$ bound; this
is consistent with the fact that the squarefree CRT structure is what
...
\texttt{verify\_sigma\_rate.py}. The contributions from Cases 1, 2,
and 3 in the proof of Theorem~\ref{thm:rate} are reported separately
to confirm both the upper bound $\sigma(N)\,N^{3}\le 2(N-2)^{2}+2\varphi(N)$
and the lower bound $\sigma(N)\,N^{3}\ge 2(N-2)^{2}-2\varphi(N)$ from
\eqref{eq:case-sum}:
...
\hline
$10$  & $4$  & $61$    & $61$    & $6$  & $120$    & $136$    & $128$\\
$30$  & $8$  & $777$   & $777$   & $6$  & $1552$   & $1584$   & $1560$\\
$210$ & $48$ & $43217$ & $43217$ & $30$ & $86432$  & $86624$  & $86464$\\
\end{tabular}
\end{center}

In every tested case, the total $\sigma(N)\,N^{3}$ lies in the
proved interval $[2(N-2)^{2}-2\varphi(N),\ 2(N-2)^{2}+2\varphi(N)]$,
with the empirical $\varepsilon(N)$ staying well below
$2\varphi(N)$. The
...
paper~\cite{SandersGish2026JCAP}.
...
\newblock Companion paper, 2026 (submitted simultaneously to
