# Cover letter — J12: The Mass Hierarchy from V⊗5 SU(5) Decomposition

**To:** Editors, *PRD*

**From:**
- B.R. Sanders (corresponding), 7Site LLC, Hot Springs, AR — brayden@7site.co
- H.J. Johnson, Independent Researcher, Billings, MT — hjj01986@gmail.com

**Date:** [DATE OF SUBMISSION]

**Manuscript title:** *The Mass Hierarchy from V⊗5 SU(5) Decomposition*

---

## Summary

[1-paragraph plain-English summary of the result]

## Why PRD

[2-3 bullet points on venue fit]

## Companion submissions

The TIG/CK research program is shipping a coordinated 55-paper sequence (J1-J55) over Summer 2026. The papers most relevant as already-submitted companions to this manuscript are:

- J10
- J23

## Reproducibility

Verification script: `Gen13/targets/ck/brain/dirac/tig_dirac.py (predict_yukawa function — TO ADD)` runs with `numpy + sympy + math` on a standard laptop in under 5 minutes.

## Suggested reviewers

[3-5 candidates appropriate to the venue]

## Conflict of interest

The authors declare no competing interests. No funding was received for this work.

---

Sincerely,
B.R. Sanders
