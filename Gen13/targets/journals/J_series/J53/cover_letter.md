...
The UOP itself is the foundational Theorem 0 of [J10] (*J. Number Theory*); this paper is the **AMM-register companion** that takes the abstract algebraic statement and turns it into a working diagnostic procedure. A live demo runs at `coherencekeeper.com/paradox.html`.
...
- **Companion to [J10] in *JNT*.** This paper is not a re-proof of the foundational theorem; it is the diagnostic procedure that the *JNT* paper enables. Two-venue strategy: *JNT* for the algebraic theorem, *AMM* for the diagnostic procedure.

## Per-venue cap note

This is the **3rd AMM submission** of the J-series, after [J21] (Q17-A 5D Force Vector) and [J20] (Mathieu $M_{22}$ Substrate-Prime). Per `J_SERIES_ORDERING.md` §5, this is at the per-venue cap. **Fallback venues** (in order): *Mathematics Magazine*, *Math. Logic Quarterly*. The manuscript can be redirected without restructuring.
