# J10 — Sprint 18 Dark Sector: Omega_b, Omega_DM, Omega_Lambda from Substrate-Operator Identities
...
The `predict_dark_sector()` function returns the three densities as exact rationals over `|Z/10|^3 = 1000`, plus the substrate derivation strings under `r['derivation']` and the Tier classification under `r['tier']`. This primitive is shared with J12 (Yukawa) via `predict_yukawa(particle, generation)` on the same module.
...
- **J03** (Sanders + Gish, JCAP) — *Logarithmic Quintessence: A Dimensionless Scalar Dark Energy Model with an Analytic Vacuum.* The JCAP companion supplies the freezing-quintessence action `S = integral d^4 x sqrt(-g) [R/(16 pi G) - (1/2) M_Pl^2 (partial Xi)^2 - Lambda^4 Xi log Xi]` from which the scale `Lambda ~ 1.74` meV is recovered as `Lambda^4 / rho_{c,0} = Omega_Lambda / 3` (J10 Theorem 6.1, matching the JCAP fit at 2.5%).
- **J06** (Sanders + Gish, Communications in Algebra) — Joint closure on Z/10 (eight-element chain + normalizer identity); supplies the `(v + h + beta + r)^2` normalizer cited in J10 §5.1.
- **J08** (Sanders + Gish, JCT-A) — sigma-rate paper; supplies the sigma-cycle length `|sigma| = 6` cited in J10 Theorem 5.2.

J10 also forward-cites **J12** (Sanders + Johnson, PRD, same Sprint 18 cluster), the Yukawa-hierarchy companion that uses the same `tig_dirac` module via `predict_yukawa()`.

## §4 — Cover letter

See `cover_letter.md` in this folder. Filled out for PRD submission with a one-paragraph plain-English summary, three venue-fit bullets, J03/J06/J08 companion list, and the `tig_dirac` reproducibility primitive.
...
- [x] Lens-scope annotation (TSML_RAW vs TSML_SYM) — N/A here; T = TSML enters only via the Z normalizer cited from J06
- [x] Cover letter finalized (J03/J06/J08 companions; tig_dirac primitive cited)
...
- [ ] Per-venue cap check: this is the **1st** PRD paper this quarter (J12 will be 2nd)
- [ ] Submitted
...
Sanders, B.R., Johnson, H.J. (2026). "Sprint 18 Dark Sector: Omega_b, Omega_DM, Omega_Lambda from Substrate-Operator Identities." Submitted to *Physical Review D*. Companion to J03 (JCAP), J12 (PRD).
