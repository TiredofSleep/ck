...
Fix the cyclotomic-embedding calibration of Definition~\ref{def:cyclo-cal} below: the major circle $S^{1}_{R}$ inherits the closed-cycle circumference of the additive flow on the prime-$p$ component of the CRT decomposition with $p$ the smallest prime divisor of $n$ at which $\deg_{\Q}(A_{p}) \le 2$, and the minor circle $S^{1}_{r}$ inherits the analogous circumference at the smallest prime $p$ (not necessarily dividing $n$) at which $\deg_{\Q}(A_{p}) \ge 3$. Under this calibration, for $n = 10$,
\[
\Tstar := R/r = 5/7.
\]
\end{theorem}
