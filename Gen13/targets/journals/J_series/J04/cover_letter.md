# Cover letter — J04: First-G Law: Squarefree Stability of the Smallest-Prime-Factor Coprime Window

**To:** Editors, *Integers*

**From:**
- B.R. Sanders (corresponding), 7Site LLC, Hot Springs, AR — brayden@7site.co
- M. Gish, Independent Researcher, Hot Springs, AR — monica.gish1992@gmail.com

**Date:** [DATE OF SUBMISSION]

**Manuscript title:** *First-G Law: Squarefree Stability of the Smallest-Prime-Factor Coprime Window*

---

## Summary

[1-paragraph plain-English summary of the result]

## Why Integers

[2-3 bullet points on venue fit]

## Companion submissions

The TIG/CK research program is shipping a coordinated 55-paper sequence (J1-J55) over Summer 2026. The papers most relevant as already-submitted companions to this manuscript are:

- J01
- J02

## Reproducibility

Verification script: `(to extract from corpus: papers/proof_first_g.py if present)` runs with `numpy + sympy + math` on a standard laptop in under 5 minutes.

## Suggested reviewers

[3-5 candidates appropriate to the venue]

## Conflict of interest

The authors declare no competing interests. No funding was received for this work.

---

Sincerely,
B.R. Sanders
