...
%%%
%%% Source: SAVE_PLAN_J03 Fork A restoration of held draft
%%%   _legacy_tiers/tier1_submit_now/_held_first_g/first_g_sinc2_FINAL.tex
%%% Surgical additions (per SAVE_PLAN §2):
...
%%% Author lane: Sanders + Gish only (Luther dropped per Brayden directive)
%%% ============================================================
...
$G_{k}(b) = \{1,\dots,k\}\setminus C_{k}(b)$. We prove two elementary
results and a synchronization between them. First, the
\emph{First-G localization theorem}: the smallest $k\ge 1$ at which
$|G_{k}(b)|>0$ is exactly the smallest prime factor $p_{1}=\spf(b)$,
and $G_{p_{1}}(b) = \{p_{1}\}$. Second, the \emph{harmonic identity}:
the squared modulus of the unit-alphabet exponential sum at frequency
$1/f$ admits the closed form
$R(k,f) = \sin^{2}(\pi k/f)/(k^{2}\sin^{2}(\pi/f))$, a discrete
$\sinc^{2}$-type function whose value is $0$ at $k=f$ and $1/(f-1)^{2}$
at $k=f-1$, and which converges to $\sinc^{2}(k/f)$ as $f\to\infty$.
The synchronization theorem records that, for $f = p_{1}$, the First-G
event and the first integer zero of $R(k,p_{1})$ coincide at $k=p_{1}$.
The proofs are elementary: a one-line gcd argument for the
First-G theorem, the geometric-series identity
$|1-e^{i\theta}|^{2} = 4\sin^{2}(\theta/2)$ for the harmonic identity,
and direct substitution for the synchronization. Verification includes
every squarefree $b$ in $[2,500]$ and every prime $f$ in $[3,23]$,
...
synchronization theorem (Theorem~\ref{thm:sync}) is the contribution:
two independently classical events coincide at $k = \spf(b)$.
Subsequent companion papers in our program (the Sanders--Gish
$\sigma$-rate paper and the four-core fusion-closure paper
\cite{SandersGish2026Sigma,SandersGish2026FourCore}) work over
...
\texttt{verify\_first\_g.py}). The exact identity
...
\paragraph{Positioning.} None of the underlying facts is new in
isolation. The First-G localization is a one-line consequence of the
definition of $\spf(b)$; the closed form for $R(k,f)$ is the standard
Fej\'{e}r calculation in finite Fourier analysis~\cite{Zygmund2002,
OppenheimSchafer2010}; the $\sinc^{2}$ continuum limit is a standard
observation. The contribution of the present note is the packaging:
a single elementary statement that synchronizes the arithmetic
First-G event with the harmonic zero of a discrete $\sinc^{2}$-type
function, valid for every $b > 1$ via $f = \spf(b)$. Related work on
the distribution of the smallest prime factor is classical
...
broader context for the coprimality-partition viewpoint adopted here.
This packaging provides a convenient bridge to related work on
composition tables over $\Zb$ \cite{SandersGish2026Sigma} and on
dimensionless scalar field models with logarithmic potentials
