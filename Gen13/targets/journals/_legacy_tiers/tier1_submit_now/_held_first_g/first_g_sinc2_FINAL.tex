...
function, valid for every $b > 1$ via $f = \spf(b)$. This packaging
provides a convenient bridge to related work on composition tables
over $\Zb$ \cite{SandersGish2026Sigma} and on dimensionless scalar
field models with logarithmic potentials
...
Squarefree-ness of $b$ is not required.
\end{remark}
...
\begin{remark}[Specific exact values]\label{rem:exactvalues}
Two evaluations of $\sinc^{2}$ that arise naturally as continuum
limits of $R(k,f)$ at rational arguments:
\begin{align*}
  \sinc^{2}(1/2) &= \frac{4}{\pi^{2}}, \\
  \sinc^{2}(1/10) &= \frac{25(\sqrt{5}-1)^{2}}{4\pi^{2}}.
\end{align*}
Both follow from $\sin(\pi/2) = 1$ and
$\sin(\pi/10) = (\sqrt{5}-1)/4$ respectively. Discrete approximants
