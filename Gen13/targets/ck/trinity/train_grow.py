...
Mechanism (real, established -- not theater):
  ...
  - GROW: when val loss plateaus and depth < MAX, append a fresh
    alpha=0 block. He extends when he needs capacity.
  - COLLAPSE: a block whose |alpha| stays near zero never earned its
    place -> prune it. He collapses what does no work. ("abilities
    earn places", at the architecture level.)
  - The SIZE CURVE (n_layers, n_params over time) is logged alongside
    the loss curve -- watch him extend and collapse.
...
PRUNE_EVERY = int(os.environ.get("CK_PRUNE_EVERY","12"))
MIN_AGE = int(os.environ.get("CK_MIN_AGE","8"))
CKPT_EVERY = 500
torch.manual_seed(7)
...
def main():
    ...
    if os.path.exists(CKPT):
        ...
        print(f"[resume] step {start}, {len(model.blocks)} layers",
              flush=True)
    ...
    for step in range(start, MAX_STEPS):
        lr = LR * min(1.0, step / WARMUP) if step < WARMUP else LR
        ...
        if step % EVAL_EVERY == 0 and step > start:
            ...
            # GROW on DIMINISHING RETURNS: window improvement < threshold
            if len(val_hist) > GROW_PATIENCE and \
                ...
                if gain < MIN_WINDOW_GAIN:
                    model.grow(); opt = rebuild_opt()
                    last_grow_eval = n_eval
                    event = f"GREW->{len(model.blocks)}L (gain {gain*100:.1f}%<{MIN_WINDOW_GAIN*100:.0f}%)"

            # COLLAPSE dead blocks (earned no alpha, old enough)
            if event is None and step % (EVAL_EVERY * PRUNE_EVERY) == 0 \
                    and len(model.blocks) > MIN_LAYERS:
                cand = [(float(b.alpha.abs()), i)
                        for i, b in enumerate(model.blocks)
                        if b.age >= MIN_AGE]
                if cand:
                    a_min, i_min = min(cand)
                    if a_min < ALPHA_DEAD:
                        model.prune(i_min); opt = rebuild_opt()
                        event = f"PRUNED#{i_min}(a={a_min:.3f})->{len(model.blocks)}L"
