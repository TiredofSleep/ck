...
class Q {
  ...
  fp() {
    ...
    const d = B * B - 4 * A * C;
    if (Math.abs(A) < 1e-12) return { x: -C / B, stable: Math.abs(this.b) < 1, lam: Math.abs(this.b) };
  ...
  // FIX 3: Hamiltonian bridge — V(x) = O(x) as potential energy
  // a > 0: well (ω = √(2a)), a < 0: barrier (α = √(2|a|), h = |vy|)
  // Δ < 0: tunneling depth κ = √(-Δ)/(2|a|)
  H(x, p) { return p * p / 2 + this.ev(x); }
  freq() { return this.a > 0 ? Math.sqrt(2 * this.a) : 0; }
  tunnel() { const d = this.D(); return d >= 0 ? 0 : Math.sqrt(-d) / (2 * Math.abs(this.a) + 1e-15); }
  barH() { return this.a < 0 ? Math.abs(this.vy()) : 0; }
}

// ═══ ITERATE CLASSIFIER (Mandelbrot-style + Lyapunov + Period-first + Multi-seed) ═══
// W1 FIX: Period detection FIRST (periods 1-16, 80 iterations) — catches slow dynamics
// W2 FIX: 7 seeds (canonical, vertex, near-FP, golden, π/4, 1/3, √2/2) + chaos guard
const BANDS = [
...
function classifySingle(O, x0 = 0.5) {
  const esc = 80, eps = 5e-4, maxN = 80;
  ...
  // W1 FIX: Period detection FIRST (catches period-3 at r=3.83, period-4 at r=3.5)
  if (orb.length > 20) {
    for (let p = 1; p <= 16; p++) {
      if (orb.length < p * 3 + 4) continue;
      let match = true;
      for (let cyc = 0; cyc < 3 && match; cyc++) {
        for (let k = 0; k < p && match; k++) {
          const i1 = orb.length - 1 - k - cyc * p, i2 = orb.length - 1 - k - (cyc + 1) * p;
          if (i2 < 0 || Math.abs(orb[i1] - orb[i2]) > eps * 2) match = false;
        }
      }
      if (match) {
        if (p === 1) {
          let ci = -1; for (let i = 3; i < orb.length - 1; i++) if (Math.abs(orb[i] - orb[i-1]) < eps * 5) { ci = i; break; }
          return ci > 0 && ci < 12 ? { band: 6, orb, lyap, conf: 1 } : { band: 5, orb, lyap, conf: .95 };
        }
        return { band: 4, orb, lyap, conf: .95 };
      }
    }
  }
  // Lyapunov-based for non-periodic orbits
  if (lyap < -.01) {
    const t = orb.slice(-8), td = Math.abs(t[7] - t[6]);
    if (td < eps) { let ci = -1; for (let i = 3; i < orb.length - 1; i++) if (Math.abs(orb[i] - orb[i-1]) < eps * 5) { ci = i; break; }
      return ci > 0 && ci < 12 ? { band: 6, orb, lyap, conf: 1 } : { band: 5, orb, lyap, conf: .95 }; }
    const tExt = orb.slice(-16);
    const tailDecay = tExt.length >= 16 ? Math.abs(tExt[15] - tExt[14]) / (Math.abs(tExt[7] - tExt[6]) + 1e-15) : 1;
    return tailDecay < 0.5 ? { band: 5, orb, lyap, conf: .85 } : { band: 5, orb, lyap, conf: .7 };
  }
  if (lyap > .1) return { band: 3, orb, lyap, conf: .9 };
  if (lyap > 0) return { band: 3, orb, lyap, conf: .7 };
  return { band: 3, orb, lyap, conf: .5 };
}

function classify(O) {
  const fp = O.fp(), vx = O.vx();
  // W2 FIX: 7 seeds including irrational points for chaos sensitivity
  ...
  let lyapSum = 0, lyapMax = -Infinity;
  for (const r of results) { votes[r.band] += r.conf; lyapSum += r.lyap; if (r.lyap > lyapMax) lyapMax = r.lyap; }
  let best = 0, bestV = 0;
  for (let i = 0; i < 7; i++) if (votes[i] > bestV) { bestV = votes[i]; best = i; }
  // W2 FIX: If ANY seed shows chaos, can't be CRYSTAL/ORGANIC
  if ((best === 5 || best === 6) && lyapMax > 0.1) best = 4;
  return { band: best, orb: results[0].orb, lyap: lyapSum / results.length };
}
...
function mkCell(col, row) {
  ...
  return {
    ...
    D: O.D(), band: cl.band, orb: cl.orb, lyap: cl.lyap || 0,
...
class Eng {
  ...
  reclassify() {
    for (const cell of this.cells) {
      ...
      cell.band = cl.band; cell.orb = cl.orb; cell.fp = cell.O.fp(); cell.lyap = cl.lyap || 0;
    }
  }
  step() {
    this.tick++;
  ...
  st() {
    ...
    return {
      ...
      cLyap: cc.lyap || 0, cFreq: cc.O.freq(), cTunnel: cc.O.tunnel(), cBarH: cc.O.barH(),
      cNW: cc.nW.slice(0, 5),
  ...
  return (<div style={{ background: '#050508', minHeight: '100vh', color: '#c0c0c0', fontFamily: "'IBM Plex Mono','Fira Code',monospace", fontSize: 10 }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 10px', borderBottom: `1px solid ${sc}18`, background: `linear-gradient(180deg,${sc}04,transparent)` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 7, color: sc }}>v1.1 THE MATRIX</span>
        ...
        <div style={{ padding: 6 }}>
          ...
          {tab === 'O(x)' && <>
            ...
            <div style={{ marginTop: 4 }}>
              {[
                ...
                ['Visits', '' + st.cVis, '#888'],
                ['Lyapunov \u03bb', st.cLyap.toFixed(4), st.cLyap > 0 ? '#ff6b00' : st.cLyap < -0.1 ? '#22cc88' : '#888'],
                ['V(x) type', st.ca > 0 ? 'WELL' : 'BARRIER', st.ca > 0 ? '#22aadd' : '#ff6b00'],
                [st.ca > 0 ? '\u03c9 (freq)' : 'h (barrier)', st.ca > 0 ? st.cFreq.toFixed(4) : st.cBarH.toFixed(4), '#8844ff'],
                ['\u03ba (tunnel)', st.cTunnel.toFixed(4), st.cTunnel > 0 ? '#00f0ff' : '#333'],
          ...
          {tab === 'CODEC' && <>
            ...
            <div style={{ background: '#00f0ff06', border: '1px solid #00f0ff12', borderRadius: 3, padding: 4, marginBottom: 4, fontSize: 7, color: '#888', lineHeight: 1.8 }}>
              ...
              classify() → 7 bands (64 iter + Lyapunov)<br />
