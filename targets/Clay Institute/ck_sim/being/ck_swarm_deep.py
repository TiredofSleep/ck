...
class SubstrateExperience:
    ...
    def confirmed_generators(self) -> List[int]:
        ...
        """
        return [op for op, count in self.discovered_generators.items()
...
class SwarmField:
    ...
    def bootstrap_from_corpus(self, texts: List[str]):
        ...
        Each text gets decomposed through D2 → core+tails → experience.
        This is NOT cheating. CK is SWARMING each text, finding the
        generators, growing his paths. Same pipeline as live input,
        just batched.

        The math: every word decomposes into 5D force → D2 curvature →
        operator classification. High curvature = core generator.
        Low curvature = tail. Consecutive cores form paths.
        After enough texts, confirmed_generators emerge naturally.
        """
        exp = self._get_experience('language')
        for text in texts:
            _, core_ops, tail_ops = decompose_text(text)
            if core_ops:
                exp.observe_decomposition(core_ops, tail_ops)
    ...
    def reflect_on_voice(self, voice_text: str,
        ...
        Returns dict:
          ...
          reflection_score: combined alignment + coherence metric
        ...
        if not voice_text or not voice_text.strip():
            return {'agent': None, 'alignment': 0.0,
                    'produced_ops': [], 'identity_agent': None,
        ...
        # Alignment: how many intended ops appear in produced ops?
        produced_set = set(produced_ops)
        overlap = produced_set & intended_set
        alignment = len(overlap) / len(intended_set)

        # Feed to identity experience (self-knowledge grows from reflection)
        id_exp = self._get_experience('identity')
        id_exp.observe_agent(agent)

        # Cross-substrate: language → identity
        id_agent = self.map_language_to_identity(agent)
        self.add_agent(id_agent)

        # Reflection score: alignment weighted by agent coherence
        reflection_score = alignment * 0.6 + agent.coherence * 0.4
