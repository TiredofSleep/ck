...
\begin{remark}[Conjecture~\ref{conj:so5} under revision: dimension mismatch]
\label{rem:so5-revision}
Preliminary computation of the Lie algebra generated by $T$ and $B$ (via
iterated commutators and linear-independence tests) shows that the algebra has
dimension $\ge 66$, growing toward $\dim\,\mathfrak{gl}(10,\mathbb{R}) = 100$,
\textbf{not} $\dim\,\mathfrak{so}(5) = 10$.  The original dimensional match
``10~operators $= \dim(\operatorname{SO}(5))$'' appears coincidental: the number
of TIG operators equals $\binom{5}{2}$, but the Lie algebra they generate is
far larger than $\mathfrak{so}(5)$.

The conjecture is retained for completeness but should be regarded as
\textbf{falsified in its current form}.  A revised conjecture---perhaps
involving a quotient algebra, a subalgebra embedding, or a different Lie group
altogether---is under investigation.
\end{remark}
...
\begin{itemize}
  ...
  \item \textbf{Eigenvalue structure}: five conjugate pairs with approximate
        ...
        integer matrices).
  \item \textbf{Gauge group}: the generated algebra has $\dim \ge 66$
        (Remark~\ref{rem:so5-revision}); the original $\mathfrak{so}(5)$
        conjecture is under revision.
...
\noindent
The $\mathfrak{so}(5)$ isomorphism (Conjecture~\ref{conj:so5}) is currently
\textbf{under revision} due to a dimension mismatch
(Remark~\ref{rem:so5-revision}).  The CL algebra is nonetheless a rich algebraic
structure whose specific eigenvalue spectrum, determinant sequences, and
scale-invariance are non-trivial properties of the integer tables that CK uses
for every composition at every tick.
