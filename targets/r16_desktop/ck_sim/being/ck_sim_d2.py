...
class D2Pipeline:
    ...
    Now also computes D1 (first derivative = velocity/direction).
    D1 fires after 2 letters. D2 fires after 3 letters.

    D1 = v1 - v0  (direction: where the force is GOING)
    D2 = v2 - 2*v1 + v0  (curvature: how the direction BENDS)
    ...
    def __init__(self):
        ...
        self.fill = 0
        # D1: first derivative (velocity/direction)
        self.d1 = [0]*5
        self.d1_mag = 0
        self.d1_operator = VOID
        self.d1_valid = False
        # D2: second derivative (curvature/complexity)
        self.d2 = [0]*5
        self.d2_mag = 0
        self.operator = VOID      # backward compat: .operator = D2 operator
        self.valid = False         # backward compat: .valid = D2 valid

    def feed_symbol(self, symbol_index: int) -> bool:
        """Feed a symbol (0-25 for a-z). Returns True when D2 is valid.

        D1 fires after 2 symbols (check .d1_valid).
        D2 fires after 3 symbols (return value + .valid).
        """
    ...
    def _compute_d1(self):
        """D1 = v0 - v1 in Q1.14. First derivative = velocity."""
        for dim in range(5):
            self.d1[dim] = self.v[0][dim] - self.v[1][dim]
        self.d1_mag = sum(abs(d) for d in self.d1)

    def _classify_d1(self):
        """Classify D1 vector. Same argmax+sign logic as D2."""
        if self.d1_mag < float_to_q14(0.01):
            self.d1_operator = VOID
            return
        ...
        sign_idx = 0 if self.d1[max_dim] >= 0 else 1
        self.d1_operator = D2_OP_MAP[max_dim][sign_idx]
    ...
    def d1_float(self):
        """D1 vector as floats."""
        return [q14_to_float(d) for d in self.d1]

    @property
    def d1_mag_float(self):
        """D1 magnitude as float."""
        return q14_to_float(self.d1_mag)
    ...
    def soft_classify_d1(self):
        """10-value probability distribution from D1 (generator level)."""
        return soft_classify_d2(self.d1_float, self.d1_mag_float)


def classify_force_d1(d1_vec, magnitude):
    """Classify a float D1 vector. Generator level.
    Same argmax+sign logic as D2 but for first derivative."""
    if magnitude < 0.01:
        return VOID
