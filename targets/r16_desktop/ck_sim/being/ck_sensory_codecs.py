...
class CurvatureEngine:
    """Compute D1 + D2 curvature on any stream of 5D force vectors.

    This is the same derivative math as the text D2 pipeline,
    but generalized to accept force vectors from any sensor.

    D1 = v[t] - v[t-1]              (first derivative: direction/generator)
    D2 = v[t-2] - 2*v[t-1] + v[t]   (second derivative: curvature/complexity)

    D1 fires after 2 vectors. D2 fires after 3.
    CL(D1, D2) = operator composition (Being x Doing = Becoming).
    """

    # Same operator map as D2Pipeline in ck_sim_d2.py
    _OP_MAP = [
    ...
    def __init__(self, history_size: int = 3):
        ...
        # D1 state (fires after 2 vectors)
        self.d1_float = [0.0] * 5
        self.d1_magnitude = 0.0
        self.d1_operator = VOID
        self.d1_valid = False
        # D2 state (fires after 3 vectors)
        self.d2_float = [0.0] * 5
        self.d2_magnitude = 0.0
        self.operator = VOID
    ...
    def feed(self, force_vec: List[float]) -> bool:
        ...
        D1 fires after 2 vectors (check .d1_valid).
        D2 fires after 3 vectors (return value).
        """
        self.history.append(force_vec[:])
        if len(self.history) > self.history_size:
            self.history.pop(0)

        # D1 = v[t] - v[t-1] (fires after 2 vectors)
        if len(self.history) >= 2:
            v_curr, v_prev = self.history[-1], self.history[-2]
            for i in range(5):
                self.d1_float[i] = v_curr[i] - v_prev[i]
            self.d1_magnitude = sum(abs(d) for d in self.d1_float)
            self.d1_operator = self._classify_argmax(self.d1_float, self.d1_magnitude)
            self.d1_valid = True
        ...
        # Classify D2: magnitude -> operator (same as D2Pipeline)
        self.d2_magnitude = sum(abs(d) for d in self.d2_float)
        self.operator = self._classify(self.d2_magnitude)
        return True

    def _classify_argmax(self, vec: List[float], magnitude: float) -> int:
        """Classify by argmax+sign (same as D2Pipeline._classify_d1).

        This is the generator-level classification: which dimension
        dominates and in which direction.
        """
        if magnitude < 0.01:
            return VOID
        max_abs = 0.0
        ...
        for dim in range(5):
            ...
            if a > max_abs:
                max_abs = a
                max_dim = dim
        sign_idx = 0 if vec[max_dim] >= 0 else 1
        return self._OP_MAP[max_dim][sign_idx]
