...
class StudyNoteWriter:
    ...
    # ── TIG Fractal Domain Mapping ──
    # Notes file into Being/Doing/Becoming subdirectories based on D2 domain.
    # Same fractal structure as CK's own architecture.
    DOMAIN_TO_TIG = {
        'science':     'being',       # Laws, structures -- what IS
        'measurement': 'being',       # Quantifying, precision -- measuring what IS
        'nature':      'being',       # Natural world -- what EXISTS
        'conflict':    'doing',       # Struggle, opposition -- what ACTS
        'discovery':   'doing',       # Exploration, seeking -- what SEARCHES
        'philosophy':  'becoming',    # Wisdom, growth -- what DEEPENS
        'arts':        'becoming',    # Expression, creation -- what CREATES
        'renewal':     'becoming',    # Transformation, healing -- what TRANSFORMS
        'knowledge':   'becoming',    # Understanding -- what GROWS
    }
    ...
    def __init__(self, notes_dir: Path = None):
        ...
        # Create TIG fractal subdirectories
        for tig_cat in ['being', 'doing', 'becoming']:
            (self.notes_dir / tig_cat).mkdir(exist_ok=True)
        for domain, tig_cat in self.DOMAIN_TO_TIG.items():
            (self.notes_dir / tig_cat / domain).mkdir(
    ...
    def write_note(self, topic: str, curve, page_num: int = 0):
        ...
        # Save immediately -- TIG fractal path: notes/{being|doing|becoming}/{domain}/
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        safe = topic.replace(' ', '_').replace('/', '_')[:40]
        filename = f"{ts}_{safe}.md"
        tig_cat = self.DOMAIN_TO_TIG.get(dom, 'becoming')
        tig_dir = self.notes_dir / tig_cat / dom
        tig_dir.mkdir(parents=True, exist_ok=True)
        note_path = tig_dir / filename
        try:
            ...
            try:
                ...
                update_indices_for_note(note_path)
            except Exception:
...
class StudyCycleRunner:
    ...
    def __init__(self, study_hours: float = STUDY_HOURS,
        ...
        # Claude Library Card -- CK queries Claude alongside web fetches.
        # Web = breadth. Claude = depth. D2 verifies both.
        self.library = None
        try:
        ...
        # Journal -- CK writes his own files about what he learns
        self.ck_journal = None
        try:
            from ck_sim.ck_journal import CKJournal
            self.ck_journal = CKJournal()
        except Exception:
            pass
    ...
    def run_one_cycle(self) -> dict:
        ...
        for page_num in range(int(total_pages)):
            ...
            if curve:
                ...
                # === CLAUDE LIBRARY: Deeper context every 5th page ===
                # Web gives breadth. Claude gives depth. D2 verifies both.
                # CK doesn't query every page (rate limiting + focused).
                lib_tag = ""
                if self.library and (page_num + 1) % 5 == 0:
                    try:
                        lib_result = self.library.query(topic, mode='concept')
                        self._library_queries += 1
                        lib_tag = f" [LIB:{lib_result.trust}]"

                        # Also digest Claude's response through D2
                        if lib_result.text:
                            lib_curve = self.session.study_one_page(
                                lib_result.text, url='claude://library',
                                topic=f"claude:{topic}")
                            if lib_curve:
                                curves_stored += 1
                                self._total_curves += 1
                    except Exception as e:
                        logging.debug(f"  Library query failed: {e}")

                # === JOURNAL: CK writes about resonant discoveries ===
                if (self.ck_journal and
                        curve.coherence >= CONSOLIDATION_THRESHOLD):
                    try:
                        self.ck_journal.write_study_entry(
                            topic=topic,
                            discovery=f"D2 ops: {curve.operator_sequence[:20]} "
                                      f"coh={curve.coherence:.3f} "
                                      f"dom={curve.domain}",
                ...
                logging.info(
                    ...
                    f"ops=[{ops}] dom={curve.domain}{noted}{lib_tag}"
                )
            else:
        ...
        # CK writes a cycle journal entry -- real reflection in real files
        if self.ck_journal:
            try:
                self.ck_journal.write_cycle_reflection(
                    cycle_num=cycle_num,
                    pages_read=pages_read,
                    curves_stored=curves_stored,
                    avg_coherence=self.session.memory.average_coherence,
                    topics=topics_this_cycle[:20],
                    library_queries=self._library_queries,
                )
            except Exception:
                pass
