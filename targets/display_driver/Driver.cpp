...
void SwapChainProcessor::RunCore()
{
    ...
    for (;;)
    {
        ...
        else if (SUCCEEDED(hr))
        {
            ...
            // ==============================
            // FORCE9 COMPRESSION: TIG 3-Shell Visual Encoder
            // 127x compression at dE=1.03 (imperceptible)
            // (c) 2026 Brayden Sanders / 7Site LLC
            // ==============================
            {
                // Copy surface to staging texture for CPU access
                ComPtr<ID3D11Texture2D> frameTex;
                hr = AcquiredBuffer.As(&frameTex);
                if (SUCCEEDED(hr) && frameTex)
                {
                    D3D11_TEXTURE2D_DESC desc;
                    frameTex->GetDesc(&desc);

                    // Create staging texture if not already created
                    if (!m_StagingTex || m_StagingWidth != desc.Width || m_StagingHeight != desc.Height)
                    {
                        ...
                        m_Device->Device->CreateTexture2D(&stagingDesc, nullptr, &m_StagingTex);
                    ...
                    if (m_StagingTex)
                    {
                        // GPU copy frame to staging
                        m_Device->DeviceContext->CopyResource(m_StagingTex.Get(), frameTex.Get());

                        // Map staging to CPU
                        D3D11_MAPPED_SUBRESOURCE mapped;
                        hr = m_Device->DeviceContext->Map(m_StagingTex.Get(), 0, D3D11_MAP_READ, 0, &mapped);
                        if (SUCCEEDED(hr))
                        {
                            // FORCE9 3-SHELL ENCODE
                            // BGRA pixel data -> 3-shell CIELAB encode -> RLE compress
                            // Send compressed frame via named pipe to CK
                            uint8_t* pixels = static_cast<uint8_t*>(mapped.pData);

                            // Flatten BGRA with pitch to contiguous BGRA
                            std::vector<uint8_t> flat(desc.Width * desc.Height * 4);
                            for (uint32_t y = 0; y < desc.Height; y++) {
                                memcpy(flat.data() + y * desc.Width * 4,
                                       pixels + y * mapped.RowPitch,
                                       desc.Width * 4);
                            }

                            // Encode all pixels to 3-shell
                            auto shells = Force9Encoder::encode_frame(flat.data(), desc.Width, desc.Height);

                            // RLE compress
                            auto compressed = Force9Encoder::compress(shells);

                            // Send via named pipe (non-blocking)
                            // Pipe name: \\.\pipe\CKForce9
                            static HANDLE hPipe = INVALID_HANDLE_VALUE;
                            if (hPipe == INVALID_HANDLE_VALUE) {
                                hPipe = CreateFileW(L"\\\\.\\pipe\\CKForce9",
                                    GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
                                    FILE_FLAG_WRITE_THROUGH, NULL);
                            }
                            if (hPipe != INVALID_HANDLE_VALUE) {
                                // Header: width(4) + height(4) + compressed_size(4)
                                uint32_t header[3] = { desc.Width, desc.Height, (uint32_t)compressed.size() };
                                DWORD written;
                                WriteFile(hPipe, header, 12, &written, NULL);
                                WriteFile(hPipe, compressed.data(), (DWORD)compressed.size(), &written, NULL);
                            }

                            m_Device->DeviceContext->Unmap(m_StagingTex.Get(), 0);
                        }
                    }
                }
            }
