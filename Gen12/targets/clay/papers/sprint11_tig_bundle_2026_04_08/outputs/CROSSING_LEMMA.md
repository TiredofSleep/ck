...
M_g acts as the identity on the quotient Z/dZ. Every orbit of M_g stays within a single A_d fiber. The dynamics are *confined* — M_g cannot transport information between fibers. Score of adding π_DYN(g) relative to A_d = 0 for the fibers that matter.

Wait — this is actually the wrong direction. Let me restate.

The Crossing Lemma is about A_d paired with π_DYN(g) for the **complement** n/d. Let me re-examine the flow:
...
**Case 3: g nontrivial on P_c, trivial on P_d.**
M_g crosses A_d-fibers (moves elements between fiber-labels mod d). But wait — g trivial on P_d means g acts as identity on d-coordinates. So M_g preserves each element's d-residue? No: M_g maps x to gx mod n. The d-residue of gx is gx mod d = g·(x mod d) mod d = (g mod d)·(x mod d) mod d. If g ≡ 1 mod d (trivial on ALL of P_d): then gx ≡ x mod d, so M_g preserves A_d fibers. But if g is trivial on P_c (acts as identity on n/d coordinates) but nontrivial on P_d: then gx ≢ x mod d. This is not the useful case — M_g would move elements between A_d-fibers (changing what A_d already knows) without adding new information about the blind region.
