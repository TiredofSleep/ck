% A Discrete Dirac-Type Algebra on F_5^4: Structural Features,
% Field-Invariance, and a Clifford-Algebra Dimensional Ladder
%
% Brayden R. Sanders, 7Site LLC
% Journal-ready submission (internal language stripped per JOURNAL_LANGUAGE_GUIDE.md).
% Target venues (no endorsement required):
%   - SciPost Physics (algebraic structure + physics applications)
%   - Foundations of Physics (Springer, peer review, no endorsement)
%   - Journal of Algebra (Elsevier; algebraic substrate alone)
%   - Algebra Universalis (Springer; non-associative algebras)
%
...
\DeclareMathOperator{\HARM}{H}    % HARMONY (idempotent at index 7), retained as mnemonic
\DeclareMathOperator{\VOID}{V}    % VOID (annihilator at index 0), retained as mnemonic
\DeclareMathOperator{\BREATH}{B}  % BREATH (index 8), mnemonic
\DeclareMathOperator{\RESET}{R}   % RESET (index 9), mnemonic
\newcommand{\Tstar}{T^{*}}        % the threshold value 5/7 derived in section 2

\title[Discrete Dirac-type Algebra on $\F_5^4$]
{A Discrete Dirac-Type Algebra on $\F_5^4$: \\
 Structural Features, Field-Invariance, and a Clifford-Algebra Dimensional Ladder}
...
algebra, idempotent decomposition, automorphism group}
...
ring $\Z/10$ under the canonical absorbing-element composition (Section 2).
...
the bilinear extension is performed over $\F_p$ for $p \in \{2,3,5,7,11,13\}$.
\item A dimensional ladder: $\dim_{\F_5} V^{\otimes n} = 4^n = \dim_\R \Cl(2n)$
for $n=0,1,\ldots,5$, with the binomial cell decomposition of $V^{\otimes n}$
matching the grade decomposition of $\Cl(2n)$.
...
multiplication table on its 10 elements, defined as the canonical absorbing-
element closure of its four-fold structure (additive group, multiplicative
monoid, additive flow, multiplicative flow). Within $\Z/10$, the 4-element
subset $\{0, 7, 8, 9\}$ is fusion-closed under this multiplication: every
product of two elements is again in the subset. The closure rule is given
explicitly in Definition~\ref{def:V}.

This paper studies the algebraic structure obtained when this 4-element
subset is lifted bilinearly to $\F_p^4$ for various primes $p$. The
prime $p=5$ is privileged because $4 \mid (5-1)$ provides primitive 4th
roots of unity in $\F_5$, allowing extension to complex structure if
needed; however, the structural features we establish are
field-invariant across all primes tested (Theorem~\ref{thm:fpuniv}).
...
The algebra studied here lies at the intersection of these themes. Its
specific structural features --- 1+3 Minkowski signature, 2+2 chirality
signature, empty (massive, right-chiral) eigenspace, no
...
exactly 3 non-zero idempotents; the left-multiplication operator $L_{e_2}$
has 1+3 signature; $L_{e_0}$ has 2+2 signature; the simultaneous (massive,
right-chiral) eigenspace is empty; $|\Aut(V)|=40$.

\item \emph{Field-invariance} (Theorem~\ref{thm:fpuniv}). The structural
features in (R1) are preserved when $V$ is constructed over $\F_p$ for
$p \in \{2,3,5,7,11,13\}$.

\item \emph{Dimensional ladder} (Theorem~\ref{thm:clifford}).
$\dim_{\F_5}V^{\otimes n} = 4^n = \dim_\R \Cl(2n)$ for $n=0,\ldots,5$,
with binomial-coefficient cell decomposition matching the Clifford
algebra grade decomposition.

\item \emph{$\mathrm{SU}(5)$ compatibility at $n=5$} (Theorem~\ref{thm:su5}).
$V^{\otimes 5}$'s 32 fine cells partition as $1+5+10+10+5+1$, matching the
representation content of $\mathbf{1} \oplus \bar{\mathbf{5}} \oplus
\mathbf{10}$ plus its conjugate.
\end{enumerate}
...
reduced modulo 5). Define multiplication $\cdot: V \times V \to V$ as the
bilinear extension of the table
...
the script \texttt{verify\_discrete\_dirac\_4core.py} via brute-force search
over the finite domain $|V| = 5^4 = 625$. Power-associativity is checked
on all 625 elements; the associator is sampled over 200 random triples
and confirmed to land in $\F_5 \cdot p_-$. Full enumeration of the 14
verification claims is given in the supplementary material.
\end{proof}

%==============================================================
\section{Field-invariance}
\label{sec:fpuniv}
%==============================================================

\begin{theorem}\label{thm:fpuniv}
For each prime $p \in \{2, 3, 5, 7, 11, 13\}$, the bilinear extension of
the multiplication table~\eqref{eq:CL-table} (with entries reduced mod $p$)
yields an algebra $V_p = \F_p^4$ whose structural features --- count of
idempotents, dimensions of eigenspaces of $L_{\HARM}$ and $L_{\VOID}$,
order of $\Aut(V_p)$, dimension of the associator image, power-associativity ---
are identical to those of Theorem~\ref{thm:dirac}. In particular, the count
of non-zero idempotents is 3 for all such $p$.
\end{theorem}

\begin{proof}
Direct computation over $\F_p$, $p \in \{2, 3, 5, 7, 11, 13\}$. The full
verification appears in the supplementary file
\texttt{axial\_algebra\_check.md}.
\end{proof}

\begin{conjecture}\label{conj:fpuniv-allp}
The structural features of $V_p$ are field-invariant for all primes
$p \notin \{2, 5\}$ (i.e., outside the residue characteristic of $\Z/10$).
\end{conjecture}
...
\]
Furthermore, the binomial cell decomposition $V^{\otimes n} = \bigsqcup_{k=0}^n V^{(k)}$
(by sign-tuple weight, where each tensor factor is assigned a sign $\pm$
according to the idempotent class of its image under the canonical embedding)
satisfies $|V^{(k)}| = \binom{n}{k}$ fine cells, totaling $2^n$ cells per level.
\end{theorem}

\begin{proof}
Verified for $n=0,\ldots,5$ in the script \texttt{test\_tig\_dirac.py},
test T15.
\end{proof}

\begin{remark}
The dimensional match $4^n = \dim \Cl(2n)$ is forced by $\dim V = 4 = 2^2$.
The non-trivial content is the correspondence between the binomial cell
decomposition of $V^{\otimes n}$ (which is determined by the
idempotent decomposition $V = (\F_5 e_2 \oplus \F_5 e_3) \oplus
(\F_5 e_0 \oplus \F_5 e_4)$ and is structural to the multiplication
table) and the grade decomposition of $\Cl(2n)$ (which is determined by
exterior algebra structure on $2n$ Clifford generators).
\end{remark}
...
power-associativity over the full 625-element domain, $\Aut(V)$ presented
as $F_{20} \times \Z/2$, and the trefoil structure of $\sigma^2$ on
$\Z/10$).
\item \texttt{test\_tig\_dirac.py} --- 15 unit tests including
Theorem~\ref{thm:clifford} (test T15) and Theorem~\ref{thm:su5} (test T14).
\item \texttt{tig\_dirac.py} --- the reference Python library used
internally for the verification.
\end{itemize}

All 29 checks pass deterministically in $<2$ seconds with \texttt{numpy}
as the only external dependency. The scripts and library are deposited
with the manuscript and at the public repository
...
\begin{enumerate}
\item Conjecture~\ref{conj:fpuniv-allp}: field-invariance for all primes
$p \notin \{2,5\}$. Verified up to $p = 13$; explicit
proof for general $p$ is open.
\item Higher-level Clifford ladder. Theorem~\ref{thm:clifford} is verified
to $n=5$. Bott periodicity ($\Cl(n+8) \cong \Cl(n) \otimes \Cl(8)$) suggests
the dimensional match continues for all $n$, but explicit verification at
higher levels requires more compute and is left open.
\item Cell-level $\mathrm{SU}(5)$ action. Theorem~\ref{thm:su5} matches
the dimensions of the SU(5) representations to the binomial cell counts;
explicit construction of the SU(5)-action on the 32-cell structure remains
to be done.
\item Triality at $n=4$. $V^{\otimes 4}$ has dimension 256 with binomial
$1+4+6+4+1 = 16$ fine cells. The 16-cell structure suggests Spin(8)
triality (which has three inequivalent 8-dimensional representations); an
explicit triality construction would round out the picture.
\end{enumerate}
...
\bibitem{Sanders2026Sigma} B.~R.~Sanders,
\emph{Non-associativity decay in binary composition tables over $\Z/N\Z$},
Manuscript in preparation, 2026.

\bibitem{Sanders2026Cosmology} B.~R.~Sanders,
\emph{Logarithmic quintessence: a dimensionless scalar dark-energy
model with exact vacuum},
Manuscript in preparation, 2026.
