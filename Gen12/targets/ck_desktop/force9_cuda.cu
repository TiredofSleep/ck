...
/* ═══════════════════════════════════════════
 * FULL GPU RLE: encode + boundaries + prefix sum + pack
 * Returns compressed bytes directly. No Python loop.
 * ═══════════════════════════════════════════ */

/* Prefix sum on boundaries to get run indices.
 * Uses simple sequential scan (fast enough for the host to do
 * since we only iterate n_runs << n_pixels after boundary detection).
 *
 * Called from host after rle_boundaries_kernel.
 */

__host__ __declspec(dllexport) int force9_rle_compress(
    Force9Context* ctx,
    uint8_t* h_out,       /* output: packed RLE bytes [2B value + 1B count] per run */
    int max_out_bytes)    /* max output buffer size */
{
    int n = ctx->n_pixels;

    /* Step 1: boundaries already computed by force9_encode (rle_boundaries_kernel).
     * Copy boundaries + f9 values to host. */
    uint8_t* h_boundaries = (uint8_t*)malloc(n);
    uint16_t* h_f9 = (uint16_t*)malloc(n * sizeof(uint16_t));

    cudaMemcpy(h_boundaries, ctx->d_boundaries, n, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_f9, ctx->d_f9, n * sizeof(uint16_t), cudaMemcpyDeviceToHost);

    /* Step 2: scan boundaries to find run starts, then pack.
     * This is sequential but only touches boundary positions (sparse). */
    int offset = 0;
    int i = 0;

    while (i < n && offset + 3 <= max_out_bytes) {
        uint16_t val = h_f9[i];
        int count = 1;
        int j = i + 1;
        while (j < n && count < 255 && h_boundaries[j] == 0) {
            count++;
            j++;
        }
        /* Pack: [2B value big-endian][1B count] */
        h_out[offset]     = (val >> 8) & 0xFF;
        h_out[offset + 1] = val & 0xFF;
        h_out[offset + 2] = (uint8_t)count;
        offset += 3;
        i = j;
    }
...
/* Combined encode + RLE in one call.
 * h_rgb: input RGB (n_pixels * 3 bytes)
 * h_out: output compressed bytes
 * max_out: max output buffer size
 * Returns: number of compressed bytes written, or -1 on error.
 * *encode_ms: filled with GPU encode time.
 */
__host__ __declspec(dllexport) int force9_encode_and_compress(
    Force9Context* ctx,
    const uint8_t* h_rgb,
    uint8_t* h_out,
    int max_out,
    float* encode_ms)
{
    ...
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(encode_ms, start, stop);
    cudaEventDestroy(start);
    cudaEventDestroy(stop);

    /* RLE pack */
    return force9_rle_compress(ctx, h_out, max_out);
}
