...
PASTORAL_PATTERNS = [
    ...
    # 2026-04-26 (Brayden): broaden patterns so more conversational
    # distress / interpersonal-difficulty / personal-direction queries
    # route to the empathic source instead of feel/field coordinates.
    # Hard conversations + interpersonal dilemmas
    r'\b(hard\s+conversation|difficult\s+conversation|tough\s+conversation|don\'t\s+know\s+how\s+to\s+tell|don\'t\s+know\s+what\s+to\s+say|need\s+to\s+talk|should\s+i\s+talk)\b',
    # Stress / overwhelm / exhaustion (not already caught)
    r'\b(stressed|stress\s+is|burned\s+out|burnt\s+out|exhausted|tired\s+all\s+the\s+time|nothing\s+feels|i\'m\s+tired)\b',
    # Self-doubt / inadequacy
    r'\b(not\s+good\s+enough|i\'m\s+a\s+failure|can\'t\s+do\s+anything\s+right|imposter|don\'t\s+belong|never\s+enough)\b',
    # Sadness explicit
    r'\b(sad|sadness|crying|in\s+tears|tears|down|low\s+mood|bad\s+day|terrible\s+day)\b',
    # Need for presence (broader than help_me already covered)
    r'\b(just\s+listen|just\s+need\s+someone|please\s+stay|don\'t\s+leave|talk\s+to\s+me|comfort\s+me)\b',
    # Confusion / lost direction
    r'\b(confused|lost\s+my\s+way|don\'t\s+know\s+where|what\'s\s+the\s+point|why\s+bother)\b',
    # Anger / frustration with personal weight
    r'\b(so\s+angry|so\s+frustrated|hate\s+my|i\s+can\'t\s+take\s+it|i\'ve\s+had\s+it)\b',
]
