...
class TestTransitionalRhConsistency(unittest.TestCase):
    """Proof 15: RH phase sign persists across observation sets.

    Grounded in: Hardy (1914), Montgomery (1973), Odlyzko (1987).
    """

    def test_verified(self):
        result = prove_transitional_rh_consistency()
        self.assertTrue(result.verified)
    ...
    def test_min_phase_gap_positive(self):
        result = prove_transitional_rh_consistency()
        self.assertGreater(result.evidence['min_phase_gap'], 0.0)
    ...
    def test_quadratic_phase(self):
        """Phase defect grows as |sigma-0.5|^2 (Hardy Z-function)."""
        result = prove_transitional_rh_consistency()
        self.assertGreater(result.evidence['off_line_phase'], 0.0)
        self.assertEqual(result.evidence['on_line_phase'], 0.0)


class TestTransitionalYmConsistency(unittest.TestCase):
    """Proof 16: YM mass gap persists across observation sets.

    Grounded in: BPST (1975), Wilson (1974), Creutz (1980).
    """

    def test_verified(self):
        result = prove_transitional_ym_consistency()
        self.assertTrue(result.verified)
    ...
    def test_mass_gap_positive(self):
        result = prove_transitional_ym_consistency()
        self.assertGreater(result.evidence['mass_gap'], 0.0)

    def test_min_gap_positive(self):
        result = prove_transitional_ym_consistency()
        self.assertGreater(result.evidence['min_gap'], 0.0)

    def test_ground_below_excited(self):
        """BPST instanton has lower defect than excited state."""
        result = prove_transitional_ym_consistency()
        self.assertLess(result.evidence['ground_delta'],
                        result.evidence['excited_delta'])


class TestTransitionalBsdConsistency(unittest.TestCase):
    """Proof 17: BSD rank coherence persists across observation sets.

    Grounded in: Gross-Zagier (1986), Kolyvagin (1990), Bhargava (2015).
    """

    def test_verified(self):
        result = prove_transitional_bsd_consistency()
        self.assertTrue(result.verified)
    ...
    def test_rank_gap_positive(self):
        result = prove_transitional_bsd_consistency()
        self.assertGreater(result.evidence['rank_gap'], 0.0)

    def test_matched_zero(self):
        """Rank-matched curves should have zero base defect."""
        result = prove_transitional_bsd_consistency()
        self.assertEqual(result.evidence['matched_delta'], 0.0)


class TestTransitionalHodgeConsistency(unittest.TestCase):
    """Proof 18: Hodge algebraicity persists across observation sets.

    Grounded in: Lefschetz (1924), Deligne (1971), Voisin (2002).
    """

    def test_verified(self):
        result = prove_transitional_hodge_consistency()
        self.assertTrue(result.verified)
    ...
    def test_separation_positive(self):
        result = prove_transitional_hodge_consistency()
        self.assertGreater(result.evidence['separation'], 0.0)

    def test_high_ratio(self):
        """Transcendental/algebraic ratio should be very large."""
        result = prove_transitional_hodge_consistency()
        self.assertGreater(result.evidence['ratio'], 10.0)

    def test_algebraic_near_zero(self):
        """Algebraic classes should have near-zero residual."""
        result = prove_transitional_hodge_consistency()
        self.assertLess(result.evidence['alg_residual'], 0.1)


class TestRunAllProofs(unittest.TestCase):
    """Test run_all_proofs top-level function."""

    def test_returns_18_proofs(self):
