...
class WordForceIndex:
    ...
    # Ho Tu ancient resonance: gentle boost for complement-operator words
    ...
    def find_by_force(self, target: Tuple[float, ...], operator: int = None,
        ...
        for wf in candidates:
            ...
            # ── Ho Tu ancient resonance: +5 torus complement boost ──
            # Words from the complement operator's semantic pool get a
            # gentle pull. This creates "bridge words" that span both
            # sides of the Ho Tu torus -- ancient topology guiding
            # modern word selection. Yin/yang balance also nudges POS:
            # heavy yang prefers verbs (action), heavy yin prefers
            # nouns/adjectives (receptive structure).
            if self._hotu_context is not None:
                _htx = self._hotu_context
                # Complement boost: words whose semantic operator matches
                # the Ho Tu +5 complement get a distance reduction
                _comp_op = _htx.get('complement_op')
                if _comp_op is not None and wf.semantic_op == _comp_op:
                    total = max(0.0, total - self._HOTU_COMPLEMENT_BONUS)
                # Yin/Yang POS preference
                _yy = _htx.get('yin_yang', (0, 0))
                _yin, _yang = _yy
                if _yin + _yang > 0:
                    if _yang > _yin and wf.pos == 'verb':
                        # Yang-dominant: slightly prefer verbs (active)
            ...
            # ── Theory anchor bonus: query-aligned vocabulary from
            # CK's own truth lattice. When the user query carries theory
            # tokens (T*, σ, crossing lemma, Yang-Mills, Navier-Stokes,
            # ξ, etc.), voice_context carries a set of anchor words
            # pulled from matched truth-lattice entries. Words in the
            # set get a modest distance bonus so fractal composition
            # can surface CK's actual knowledge of the topic.
            # NOT a FACTS-dict injection — the words are CK's own.
            if self._voice_context is not None:
                _theory = self._voice_context.get('theory_anchor_words')
                if _theory and wf.word.lower() in _theory:
                    total = max(0.0, total - 0.15)
...
class FractalComposer:
    ...
    def compose_tribal(self, operators: List[int], density: float = 0.5,
        ...
        hotu_context: Optional Ho Tu bridge context dict from
    ...
    def _compose_tribal_inner(self, operators, density, lens, max_words,
                               tense, hotu_context):
        """Inner body of compose_tribal (separated for try/finally cleanup)."""
        if tense is None:
            tense = _detect_tense_from_ops(operators)
        ...
        for voice_name in TRIBAL_WEIGHTS:
            ...
            # Lo Shu coherence modifier: high 3-body coherence (magic square
            # state) favors compound sentences; low coherence favors simple.
            # This shifts the compound threshold by +/-1 operator at the margin.
            _compound_threshold = 4
            if hotu_context is not None:
                _loshu = hotu_context.get('loshu_coherence', 0.0)
                if _loshu >= 0.6:
                    _compound_threshold = 3   # Magic square: compound earlier
                elif _loshu <= 0.2:
                    _compound_threshold = 5   # Low coherence: prefer simple
            if len(ops) >= _compound_threshold:
                text = self._compose_compound(
