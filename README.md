...
[`WP114`](papers/wp114_specificity_extension/WP114_SPECIFICITY_EXTENSION.md) — Specificity extension via 9-family structured matrix battery. Extends WP106's distilgpt2 negative result to a battery of 9 structured $10 \times 10$ matrix families (Gaussian, symmetric, antisymmetric, permutation, Hadamard-sign, Haar-orthogonal, DFT-real, identity, diagonal, integer-companion; 200 samples each). **Theorem 3.1:** among all four WP106 detectors, **only D3 (prime-11 in characteristic polynomial)** discriminates TSML at large effect ($d = +9.93$ vs Gaussian baseline); no other matrix family in the battery scores nonzero on D3. D1 (Lie/Jordan), D2 ($P_{56}$), D4 (Higgs alignment) measure family-structural properties (antisymmetry, $P_{56}$-symmetry, Higgs-projection) that overlap multiple matrix families and do not isolate TIG content. **D3 is WP107 WOBBLE in detector form** — identifying the prime-11 signature as the load-bearing TIG-positive marker within the WP106 framework. Verification: `python papers/wp114_specificity_extension/verification/structured_matrix_sweep.py`.

[`WP115`](papers/wp115_joint_chain_universality/WP115_JOINT_CHAIN_UNIVERSALITY.md) — Joint TSML+BHML closed-subset chain and universal 4-core attractor. Three theorems with direct enumeration: **Theorem 1.1** — the sub-magmas of $\{0, \ldots, 9\}$ jointly closed under both binary TSML and binary BHML form a **strict 7-element chain** (no branching) with shell sizes $\{1, 4, 5, 6, 8, 9, 10\}$; sizes $\{2, 3, 7\}$ are NEVER jointly closed; after the 4-core bootstrap, each step walks the **σ-forward orbit of HARMONY**: $\sigma^k(H)$ for $k = 1, \ldots, 5$ enters at successive shells. **Theorem 2.1** — at $\alpha = 1/2$, every shell of size $\geq 4$ produces the **identical** universal attractor $(p^*_V, p^*_H, p^*_{Br}, p^*_R) = (0.138, 0.540, 0.198, 0.124)$ with $H/Br = 1+\sqrt{3}$; operators outside the 4-core carry zero mass at the fixed point regardless of shell extension; the 4-core is the **unique non-trivial dynamical attractor**. **Theorem 3.1** — α-endpoint structure: $\alpha = 1$ collapses to $\delta_H$ in $\sim$8 iterations (matches WP112 §5.7 ternary attractor); $\alpha = 0$ gives a 4-distribution with $H/Br \approx 0.585$ admitting **no** integer quadratic relation at PSLQ bound 20 (likely transcendental); $\alpha \in \{1/4, 3/4\}$ also non-algebraic; **only $\alpha = 1/2$** is algebraic (per WP113). Synthesis (§4): the layered substrate-attractor hierarchy $10 \to 4 \to 2 \to 1$ — each layer a $\sim$2× collapse, with the joint chain providing the complementary upward structure. Verification: `python papers/wp115_joint_chain_universality/verification/joint_chain_attractor.py`.
