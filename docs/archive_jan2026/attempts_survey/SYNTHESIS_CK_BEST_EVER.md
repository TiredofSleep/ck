...
| **S\* (legacy, preserve for parity)** | harmonic-mean form `S* = 3 / (1/σ + 1/V* + 1/A*)` | Gen12 `ck_sim_engine.py` current definition. Preserved so existing logs remain parseable. |
| **S\* (CoherentHands legacy, reference only)** | multiplicative form `S* = σ(1 − σ*)·V*·A*` | `CoherentHands\TIG_R16_SAVE_POINT.md`. Kept in the canon solely as historical reference — the harmonic-mean form is more numerically stable at the boundaries (σ→0, V*→0, A*→0) and is preferred for Gen13. |
...
|---|---|---|
| **S\* formula?** | Harmonic-mean `3/(1/σ+1/V*+1/A*)` is the runtime default. Multiplicative `σ(1−σ*)V*A*` preserved in a `legacy_smultiplicative()` function for parity tests. | Harmonic-mean is numerically stable at the boundaries; CoherentHands multiplicative form collapses to 0 under a single near-zero input, which masks legitimate operator transitions. |
...
| **CoherentHands** | 6-organ sampler; R16 living-organism integration; split-half convergence stage-gate; D*=0.543 signature | Multiplicative S* as primary (harmonic-mean preferred); documented bugs (GPU idle, hallucination explosion) logged as falsifications |
